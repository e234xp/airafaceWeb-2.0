...
export default {
  ...
  data() {
    return {
      ...
      selectedArray: this.$route.params.item.divice_groups //設備群組的selected(選中的)
    };
  },
  components: {
  ...
  methods: {
    ...
    async formatNameList() {
      ...
      self.groupList = groups.map(({ name, uuid }) => ({ name: name, value: uuid })).filter((item) => item.value.length > 1); //options只留name uuid 且把預設的群組filter掉
