  ...
  methods: {
    ...
    convertToNewDataStructure(oldDataList) {
      ...
      Object.keys(oldDataList).forEach((key) => {
        if (key === 'person') {
          // 處理人員欄位
          Object.keys(oldDataList[key]).forEach((personKey) => {
            if (oldDataList[key][personKey]) {
              personFields.push(personKey);
            }
          });
        } else if (key === 'display_image') {
          // display_image 也加入事件欄位陣列
          eventFields.push(oldDataList[key]);
        } else if (oldDataList[key] === true) {
          // 將其他事件欄位加入陣列
          eventFields.push(key);
        }
      });

      if (eventFields.length > 0) {
        eventData.selectedFields = eventFields;
      }
      if (personFields.length > 0) {
        personData.selectedFields = personFields;
      }
