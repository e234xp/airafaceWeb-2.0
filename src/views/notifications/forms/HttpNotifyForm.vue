        ...
        <CCardBody>
          <table class="table-layout">
            ...
            <tr class="table-tr">
              ...
              <td class="table-td">
                <CInput size="lg" v-model="value_password" :type="flag_view_password ? 'text' : 'password'">
            ...
            <tr class="table-tr">
              <th class="h5 w-25 table-th" colspan="4">
                {{ $t('HttpUrl') }}
                <span class="h6 table-td px-3 py-0" style="vertical-align: bottom;">ex:
                  http://127.0.0.1:7001/api/createEvent</span>
              </th>
            </tr>
            <tr class="table-tr">
              <td class="table-td" colspan="4">
                <CInput size="lg" v-model="value_url" required :invalid-feedback="$t('NoEmptyNoSpaceOnly')"
                ...
                <ul class="list-group">
                  <li class="list-group-item" v-for="(item, index) in param_Fields" :key="index">
                    <input class="form-check-input me-1" type="checkbox" value="item"
                      :checked="value_selectedFields.indexOf(item) >= 0" @change="fieldSelectedChanged(item, $event)">
                    <input size="lg" style="width: 260px;" :value="item.key" @input="fieldChanged(item, $event)"> = {{
                    $t(item.field) }}
