...
const defaultlState = () => ({
  ...
  disp_header: i18n.formatter.format('PersonReport'),
      ...
      try {
        this.value_allTableItems.forEach((pItem) => {
          ...
          try {
            ...
            const showimageId =
              item.face_image_id && item.verify_mode !== 2 ? item.face_image_id.f + item.face_image_id.uuid : '';
            item.showimage =
              `<img id='${showimageId}' src='data:image/jpeg;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAf` +
      ...
      for (let idx = 0; idx < self.value_allTableItems.length; idx += 1) {
        ...
        if (self.exportNo % 1000 === 0) {
          await workbook.xlsx.writeBuffer().then((data) => {
            ...
            zip.file(`Investigation-Report-${self.exportNo / 1000}.xlsx`, blob);
          });
