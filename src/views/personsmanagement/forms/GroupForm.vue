...
export default {
  ...
  mounted() {
    ...
    self.downloadPersonAsync(6, function (error, reset, more, tableItems) {
      if (!error) {
        if (reset) {
    ...
    self.downloadVisitorAsync(6, function (error, reset, more, tableItems) {
      if (!error) {
        if (reset) {
  ...
  methods: {
    ...
    updateData() {
      ...
      const selectedItems = this.value_selectedPersonList.concat(
        this.value_selectedVisitorList
      );

      if (self.flag_currentSelection == 0) {
        allItemsToShow = self.value_allPersonList;
        selectedItems.forEach((seletedItem) => {
          allItemsToShow = allItemsToShow.filter(function (item) {
            return item.uuid !== seletedItem.uuid;
          });
        });
      } else {
    ...
    clickOnInsert() {
      ...
      if (this.flag_currentSelection == 0)
        ...
        this.value_selectedVisitorList = this.value_selectedVisitorList.concat(
          allRecords
        );
      this.updateData();
    },
    clickOnRemove() {
      ...
      selectedRecords.forEach((seletedItem) => {
        self.value_selectedPersonList = self.value_selectedPersonList.filter(function (
          item
        ) {
          return item.uuid !== seletedItem.uuid;
        });
        self.value_selectedVisitorList = self.value_selectedVisitorList.filter(function (
          item
        ) {
          return item.uuid !== seletedItem.uuid;
        });
    ...
    groupNameValidator(val) {
      ...
      if (val.replace(/\s/g, "").length == 0) {
        ...
        self.flag_groupNamePass = val.length > 0;
      }
    ...
    selectAllEvent_nonsel({ checked, records }) {
      // console.log(checked ? '所有勾选事件' : '所有取消事件', records)
    },
    ...
    generateSelectedFilteredData(sourceData, filter) {
      ...
      const sliceList = filteredItems.slice(
        (self.value_selectedTablePage.currentPage - 1) *
          self.value_selectedTablePage.pageSize,
