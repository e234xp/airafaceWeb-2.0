...
export default {
  ...
  data() {
    return {
      ...
      value_selectedDateName: [this.$t('Today'), this.$t('Yesterday'), this.$t('Select')],
  ...
  mounted() {
    ...
    self.$globalGetAttendanceSettings((err, data) => {
      if (!err) {
        ...
        self.value_workingHourSettings.videoDeviceGroupIn = [];
        self.value_workingHourSettings.videoDeviceGroupOut = [];

        if (
          self.value_workingHourSettings.video_device_group_in.length >= 1 ||
          self.value_workingHourSettings.video_device_group_out.length >= 1
        ) {
          self.$globalFindVideoDeviceGroups('', 0, 2000, (error, response) => {
            if (error == null) {
  ...
  methods: {
    ...
    clickOnDetails(item) {
      ...
      if (item.attendance_data_list && item.attendance_data_list.length > 0) {
        this.value_attendanceDataListToReview = item.attendance_data_list.sort((a, b) => a.timestamp - b.timestamp);
        this.value_dataItemsToShowDetailData = this.generateFilteredDataForDetailData(
          this.value_attendanceDataListToReview,
          this.value_searchingFilter,
        );
    ...
    async exportMaterToExcel() {
      ...
      if (this.value_attendanceDataListToReview == null) {
        ...
        for (let i = 0; i < this.value_masterexportFields.length; i += 1) {
          switch (this.value_masterexportFields[i]) {
            ...
            case 'group_list':
              ...
              break;
            case 'timestamp':
              columns.push({ header: this.$t('Date'), key: 'clockDate', width: 10 });
              break;
            case 'workingTime':
            ...
            case 'overTime':
              ...
              break;
            case 'clockinTime':
              columns.push({ header: this.$t('ClockTime'), key: 'clockin', width: 10 });
              break;
            case 'clockinTemperature':
              columns.push({ header: this.$t('Temperature'), key: 'clockin_temperature', width: 10 });
              break;
            case 'clockoutTime':
              columns.push({ header: this.$t('ClockOutTime'), key: 'clockout', width: 10 });
              break;
            case 'clockoutTemperature':
              columns.push({ header: this.$t('Temperature'), key: 'clockout_temperature', width: 10 });
              break;
            default:
              break;
          }
        ...
        for (let idx = 0; idx < allData.length; idx += 1) {
          this.exportNo += 1;
          this.excelCounter += 1;
        ...
        for (let i = 0; i < this.value_masterexportFields.length; i += 1) {
          switch (this.value_masterexportFields[i]) {
            ...
            case 'group_list':
              ...
              break;
            case 'timestamp':
              ln.push(`${dayjs(this.value_specifiedDate).format('MM/DD')}`);
              break;
            case 'workingTime':
            ...
            case 'overTime':
              ...
              break;
            case 'clockinTime':
              ln.push(`${item.clockin}`);
              break;
            case 'clockinTemperature':
              ln.push(`${item.clockin_temperature}`);
              break;
            case 'clockoutTime':
              ln.push(`${item.clockout}`);
              break;
            case 'clockoutTemperature':
              ln.push(`${item.clockout_temperature}`);
              break;
            default:
              break;
          }
    ...
    async exportDetailToExcel() {
              ...
              if (imageRet && imageRet.data) {
                ...
                if (this.value_snapshotFileType === 'Files') {
                  const fileTime = this.value_attendanceDataListToReview[idx].clockTime
                    ...
                    .replace(' ', '_')
                    .replace(':', '_');
                  snapshotFolder.file(`${this.exportNo}_${fileTime}.jpeg`, imageRet.data.face_image, { base64: true });
                }
              }
            }
          }
      ...
      if (workbook != null) {
        workbook.xlsx.writeBuffer().then((data) => {
          ...
          zip.generateAsync({ type: 'blob' }).then((content) => {
            FileSaver.saveAs(content, `Attendance_${dayjs(this.value_specifiedDate).format('MMDD')}_${personName}.zip`);
          });
        });
      }
    ...
    generateFilteredData(sourceData, filter) {
      ...
      this.value_showAllPerson = `${this.$t('AllPerson')} : ${this.value_allTableItems.length}`;
    ...
    generateFilteredDataForDetailData(sourceData, filter) {
      ...
      sliceList.forEach((pItem) => {
        ...
        if (showimageId.length > 0) {
          item.showimage =
            `<img id='${showimageId}' src='data:image/jpeg;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAA` +
    ...
    async refreshTableItems(cb) {
      if (this.onFetchPersonDataCallback) {
        ...
        const startTime = new Date(this.value_specifiedDate);
        startTime.setHours(0, 0, 0, 0);
        const endTime = new Date(this.value_specifiedDate);
        endTime.setHours(23, 59, 59, 999);

        const summaryRet = await this.$globalGetAttendanceSummary(startTime.getTime(), endTime.getTime());
        ...
        // 根據篩選模式決定查詢方式
        if (this.value_filterMode === 'all') {
          // 全部模式：使用原本的分頁查詢
          const sliceShift = (this.value_tablePage.currentPage - 1) * this.value_tablePage.pageSize;
          const sliceSize = this.value_tablePage.pageSize;
          ...
          this.onFetchPersonDataCallback(
            ...
            async (error, personTableItems, totalLength) => {
              if (!error) {
                // 設定總筆數
                this.value_tablePage.totalResult = totalLength;
                ...
                if (personTableItems && personTableItems.length >= 1) {
                  ...
                  this.onFetchPersonAttendanceDataCallback(
                    ...
                    (errorOnPersonVr, resetPersonVr, morePersonVr, personVrItems) => {
                          ...
                          dataListOnUuid.forEach((d) => {
                            if (!personCopy.attendance_data_list.find((att) => att.verify_uuid === d.verify_uuid)) {
                              personCopy.attendance_data_list.push({
                                ...
                                source_id: d.source_id,
                                temperature:
                                  d.temperature === 0 || d.temperature === '' ? '' : `${d.temperature.toFixed(1)}°C`,
                                verify_mode: d.verify_mode,
                                verify_mode_string: d.verify_mode_string,
                                verify_score: d.verify_score,
                                card_number: d.card_number,
                      ...
                      // 處理當前頁資料並顯示
                      this.value_dataItemsToShow = this.generateFilteredDataForCurrentPage(currentPageItems);
          ...
          const startIndex = (this.value_tablePage.currentPage - 1) * this.value_tablePage.pageSize;
          const endIndex = startIndex + this.value_tablePage.pageSize;
          const currentPageUuids = this.value_filterUuidList.slice(startIndex, endIndex);
    ...
    generateAttendanceStatusData_V2(workingHourSettings, pItem) {
      ...
      const startTimeOfMonth = new Date(
        this.value_specifiedDate.getFullYear(),
        this.value_specifiedDate.getMonth(),
        1,
        0,
        0,
        0,
      ...
      const endTimeOfMonth = new Date(
        this.value_specifiedDate.getFullYear(),
        this.value_specifiedDate.getMonth() + 1,
        1,
        0,
        0,
        0,
    ...
    generateAttendanceStatusTable(attendanceData) {
      ...
      headerDataString +=
        `<th style='text-align:center;opacity: 1.0;width:150px;font-size: 10px;border: 1px solid ${tableBordderColor};` +
        `text-overflow: ellipsis; overflow: hidden; font-family:Arial;padding: 0px 0px; color: #606266; background-color:${colorToAppend}'>`;
      headerDataString += `<div style='height:25px;margin:-5px 0px'>${dayjs(this.value_specifiedDate).format(
        'YYYY-MM-DD',
      )}</div>`;
      headerDataString += '</th>';
      ...
      if (!isHoliday) {
        clockInStatusColor = this.value_attendanceStatusColor[attendanceData.attendance_data.clock_in_status[i]];
      ...
      if (!isHoliday) {
        clockOutStatusColor = this.value_attendanceStatusColor[attendanceData.attendance_data.clock_out_status[i]];
