<template>
  <div>
    <div>
      <CCol sm="12" class="page_header">
        <td class="h1">{{ disp_header }}</td>
      </CCol>
    </div>

    <div>
      <CCol sm="12" class="page_operation">
        <CRow>
          <div>
        ...
        <div>
          <div id="LineNotifyManagementFrom">
            <vxe-table stripe ref="mainTable" align="center" :data="value_dataItemsToShow" :cell-style="cellStyle"
              ...
              <vxe-table-column field="name" width="20%" :show-overflow="ellipsisMode" :title="disp_name" />
              <vxe-table-column field="access_token" width="20%" :show-overflow="ellipsisMode"
                :title="disp_access_token" />
...
const defaultlState = () => ({
  ...
  disp_header: i18n.formatter.format('LineNotifyManagement'),
  ...
  disp_access_token: i18n.formatter.format('LineAccessToken'),
...
export default {
  ...
  methods: {
    ...
    async clickOnSwitch(event, row) {
      ...
      await self.$globalModifyLineNotify(settingData);
    },
