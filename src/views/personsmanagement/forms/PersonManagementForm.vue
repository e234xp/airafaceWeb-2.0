  ...
  methods: {
    ...
    async generateFilteredData(sourceData, filter) {
      ...
      sliceList.forEach((item) => {
        ...
        sliceItem.imgRegisterPhoto = this.formatBase64ToImgTag(`imgRegisterPhoto${sliceItem.uuid}`, 100, 100, '');
        sliceItem.imgDisplayPhoto = this.formatBase64ToImgTag(`imgDisplayPhoto${sliceItem.uuid}`, 100, 100, '');
    ...
    personInfoListener(pEvent) {
      ...
      myReader.onload = async () => {
        ...
        me.value_excelFileImpport = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]]);
    ...
    async clickOnImport() {
      ...
      async function batchEnroll(pItem, resolve) {
        ...
        const proformEnroll = () => {
          const beginDate =
            localItem['Begin Date'] && localItem['Begin Date'].length > 0
              ? new Date(localItem['Begin Date'].replaceAll('-', '/')).getTime()
              : 0;
          const expireDate =
            localItem['Expire Date'] && localItem['Expire Date'].length > 0
              ...
              const dataForModify = {
                ...
                data: {
                  ...
                  extra_info: {
                    ...
                    department:
                      data.extra_info.department === '' ? person.extra_info.department : data.extra_info.department,
                    email: data.extra_info.email === '' ? person.extra_info.email : data.extra_info.email,
                    phone_number:
                      data.extra_info.phone_number === ''
                        ? person.extra_info.phone_number
                        : data.extra_info.phone_number,
                    extension_number:
                      data.extra_info.extension_number === ''
                        ? person.extra_info.extension_number
                        : data.extra_info.extension_number,
    ...
    async clickOnExport(withPhoto) {
      ...
      if (salf.value_selectedItems != null) {
        ...
        for (let idx = 0; idx < salf.value_selectedItems.length; idx += 1) {
          ...
          if (withPhoto) {
            const photoRet = await salf.$globalFetchPhoto(salf.value_selectedItems[idx].uuid);
    ...
    clickOnMultipleDelete() {
      const list = [...this.$refs.mainTable.getCheckboxReserveRecords(), ...this.$refs.mainTable.getCheckboxRecords()];
