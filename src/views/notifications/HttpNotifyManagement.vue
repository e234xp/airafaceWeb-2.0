<template>
  <div>
    <CRow>
      <CCol sm="12">
        <CHttpNotifyManagementForm :form-data="$data" :on-add="onAdd" :on-modify="onModify" :on-delete="onDelete"
    ...
    methods: {
      async downloadTableItemsAsync(sliceSize, cb) {
        ...
        while (this.flag_keepingDownload && thereIsMoreData) {
          const ret = await this.$globalGetHttpNotifyList(
            '', shitf, sliceSize,
          );
      ...
      async removeRecordAsync(uuid, cb) {
        const ret = await this.$globalRemoveHttpNotify(uuid);
        const err = ret.error;
        if (err) {
          if (cb) cb(false);
          this.$fire({
