    ...
    <CCol sm="12">
      ...
      <CCard>
        ...
        <CCardBody>
          <CCol sm="12">
            <CRow>
              <CButton class="btn btn-primary btn-w-normal fz-lg" @click="clickOnGenerateBackup"
                :disabled="flag_generating || flag_downloading">
                {{ disp_generateBackup }}
              </CButton>

              <CButton class="btn btn-primary btn-w-normal fz-lg ml-5" @click="clickOnDownload"
                :disabled="flag_downloading || !value_lastDbBackupTime">
                {{ disp_download }}
              </CButton>

              <div v-if="flag_downloading">
                <progress-bar :options="progressOptions" :value="value_downloadProgress"
          ...
          <CCol sm="12">
            <CRow>
              ...
              <div v-if="flag_uploading">
                <progress-bar :options="progressOptions" :value="value_uploadProgress"
                  style="margin: -22px 20px; padding: 0" />
              </div>
            </CRow>
          </CCol>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async checkDbbackup() {
        const self = this;
        const ret = await self.$globalCheckDbBackupFile();
        console.log('checkDbbackup', ret.data);
        if (!ret.error) {
          if (ret.data.date.length > 0) {
            self.value_lastDbBackupTime = ret.data.date;
            self.disp_dbBackupInfo = i18n.formatter.format('DbBackupTime') + self.value_lastDbBackupTime;
          } else {
            self.value_lastDbBackupTime = null;
            self.disp_dbBackupInfo = '';
          }
      ...
      async downloadDb(cb) {
        ...
        let error = null;
        const token = self.$globalGetTokenString();
        ...
        await client
          ...
          .then((response) => {
            ...
            link.download = `airatabletlite_db_backup_${self.yyyy_mm_dd_HH_MM_SS(new Date())}.dbf`;
            link.click();
      ...
      CheckGenerateBackup(query) {
        const self = this;
        self.$globalGenerateDbBackup(query, async (err, data) => {
          if (!err) {
            if (data.message !== 'ok') {
