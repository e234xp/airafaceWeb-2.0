          ...
          <CCol sm="12">
            ...
            <CRow>
              <CCol sm="3">
                ...
                <CInput size="lg" v-model="value_ntpServerIpAddress" :disabled="!value_ntpServerActivate">
                  <template #append-content>
                    <div style="width:55px;margin:0 0 -5px 0">
                      <CSwitch color="success" shape="pill"
          ...
          <CCol sm="12">
            <CRow>
              <CCol sm="3">
                ...
                <CSelect size="lg" :value.sync="value_selectedTimezone" :options="value_avaiableTimezoneListToShow"
                  @change="setTimezone($event)" />
              </CCol>
              <CCol sm="3">
                <div style="height:30px;" />
                <CButton style="height:45px;width: 300px;background-color:#20a8d8;color: white;"
                  @click="clickOnApplyTimeSettings" :disabled="flag_applying">
                  <div v-if="value_ntpServerActivate" style="font-size:20px">
    ...
    created() {
      ...
      if (this.value_timeoutID == null) {
        this.value_timeoutID = window.setInterval(() => {
          ...
          this.value_currentLocalTimeString = `( ${window.Intl.DateTimeFormat().resolvedOptions().timeZone} ) ${localTime.getFullYear()}-${(localTime.getMonth() + 1).toString().padStart(2, 0)}-${localTime.getDate().toString().padStart(2, 0)} ${localTime.getHours().toString().padStart(2, 0)}:${localTime.getMinutes().toString().padStart(2, 0)}:${localTime.getSeconds().toString().padStart(2, 0)}`;
          ...
          if (this.value_deviceTimezone.length > 0) {
            ...
            this.value_currentDeviceTimeString = `( ${this.value_deviceTimezone} ) ${deviceTime.getFullYear()}-${(deviceTime.getMonth() + 1).toString().padStart(2, 0)}-${deviceTime.getDate().toString().padStart(2, 0)} ${deviceTime.getHours().toString().padStart(2, 0)}:${deviceTime.getMinutes().toString().padStart(2, 0)}:${deviceTime.getSeconds().toString().padStart(2, 0)}`;
