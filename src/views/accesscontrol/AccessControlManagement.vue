        ...
        <div>
          <div id="personManagementFrom">
            <vxe-table :data="value_dataItemsToShow" stripe align="center" :cell-style="cellStyle"
              ...
              <vxe-table-column type="checkbox" min-width="10%" align="center"></vxe-table-column>
    ...
    methods: {
      ...
      generateFilteredData(sourceData, filter) {
        ...
        const filteredItems =
          filter.length == 0
            ? sourceData
            : sourceData.filter((item) => item.name.toLowerCase().indexOf(filter.toLowerCase()) > -1
              || item.group_list.toString().indexOf(filter.toLowerCase()) > -1
              || item.access_control_type_name.toLowerCase().indexOf(filter.toLowerCase()) > -1);
