<template>
  <div class="ratio-wrap ratio-wrap-16x9" v-show="!isLoadSetting">
    <div class="ratio-content dashboard" style="position: absolute; z-index: 50;"
      ...
      <div class="guard-content" style="display: flex; width: 100%;"
        :style="{ height: `calc(100% - ${61 * zoomRatio}px)` }">
        <!-------------------  Guard - LEFT ------------------>
        <div style="width: 100%; padding: 24px; position: relative;" :style="'zoom: ' + zoomRatio + ' !important;'">
          <div style="display: flex; width: 100%; height: 240px; background: #2C2C2C; margin-bottom: 16px;"
            ...
            <div class="fz-xxxl"
              ...
              <div>
                <span class="fz-super-large" style="color: #7FB97A">{{ entryPersons.length }}</span> {{ $t('UnitPerson')
                }}
              </div>
            </div>
          ...
          <template v-if="expandFlag === ''">
            <GuardRegionTitle :index="currentPageIndex" :total="currentPageTotal" :expand="false" @prev="onCurrentPrev"
              @next="onCurrentNext" @expand="onExpand('current')" type="present">
              {{ $t('PersonOfAdmission') }}
            </GuardRegionTitle>
            <div class="grid-2-6" style="height: 240px; margin-bottom: 16px;">
              <FaceCard type="present" :image="parseImage(item)" :name="parseLine1(item)" :depart="parseLine2(item)"
                :time="item.clockinRecord.timestamp" v-for="item in currentPersons" :key="item.uuid" />
            </div>
            <GuardRegionTitle :index="strangerPageIndex" :total="strangerPageTotal" :expand="false"
              @prev="onStrangerPrev" @next="onStrangerNext" @expand="onExpand('stranger')" type="unknown">
              <div style="display: flex; gap: 16px">
                ...
                <div class="chart-btn fz-lg" :class="[ selectedStrangers.length === 0 ? 'disabled' : '' ]"
                  style="padding: 0 16px; margin-left: auto" @click="onBatchAck(false)">
                  {{ $t('Remarks') }}
                </div>
                <div class="chart-btn fz-lg" :class="[ selectedStrangers.length === 0 ? 'disabled' : '' ]"
                  style="padding: 0 16px;" @click="onBatchAck(true)">
                  {{ $t('Acknowledge') }}
                </div>
              </div>
            </GuardRegionTitle>
            <div class="grid-2-6" style="height: 240px;">
              <FaceCard type="unknown" :image="item.face_image" :time="item.timestamp"
                :selected="isSelected(item.verify_uuid)" @ack="onAck(item.verify_uuid)"
                @click.native="onSelect(item.verify_uuid)" v-for="item in currentStrangers" :key="item.verify_uuid" />
            </div>
          </template>
          <template v-if="expandFlag === 'current'">
            <GuardRegionTitle :index="currentPageIndex" :total="currentPageTotal" :expand="true" @prev="onCurrentPrev"
              @next="onCurrentNext" @expand="onExpand('')" type="present"
          ...
          <template v-else-if="expandFlag === 'stranger'">
            <GuardRegionTitle :index="strangerPageIndex" :total="strangerPageTotal" :expand="true"
              @prev="onStrangerPrev" @next="onStrangerNext" @expand="onExpand('')" type="unknown"
              style="position: absolute; top: 16px; left: 24px; width: calc(100% - 40px)">
              <div style="display: flex; gap: 16px">
                <div style="margin-right: 16px;">
                  {{ $t('Stranger') }}
        ...
        <!-------------------  Guard - RIGHT ------------------>
        <div style="width: 20%; height: 100%; background: rgba(44, 44, 44, 0.4); padding: 16px; color: white"
          :style="'zoom: ' + zoomRatio + ' !important;'" v-if="false">
          <div class="fz-xxxl" style="margin-bottom: 24px">
            即時影像
          </div>
    ...
    <GuardAckModal :style="'zoom: ' + zoomRatio + ' !important;'" :persons="ackedStrangers" :direct="autoAck"
      @close="onClose" @confirm="onConfirm" v-if="showAckModal" />
