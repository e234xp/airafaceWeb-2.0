<template>
  <div>
    <CRow>
      <CCol sm="12">
        <CActionRuleManagementForm :formData="$data" :onAdd="onAdd" :onModify="onModify" :onDelete="onDelete"
...
<script>
  ...
  import CActionRuleManagementForm from './forms/ActionRuleManagementForm.vue';

  export default {
    name: 'ActionRuleManagement',
    components: { CActionRuleManagementForm },
    data() {
      return {
        type: 'ActionRule',
    ...
    beforeRouteLeave(to, from, next) {
      console.log('ActionRuleManagement', to, from, next);
    ...
    methods: {
      async downloadTableItemsAsync(sliceSize, cb) {
        ...
        while (self.flag_keepingDownload && thereIsMoreData) {
          const ret = await self.$globalGetActionRuleList(
            '', shitf, sliceSize,
          );
      ...
      onAdd(allRecords) {
        this.$router.push({
          ...
          params: {
            ...
            value_returnRoutePath: 'ActionRuleManagement',
      ...
      async removeRecordAsync(uuid, cb) {
        const self = this;
        const ret = await self.$globalRemoveActionRule(uuid);
        const err = ret.error;
        if (err) {
          if (cb) cb(false);
          self.$fire({
