					...
					<CRow class="justify-content-between buttons-group">
						...
						<div class="d-flex">
							...
							<date-picker :lang="this.$globalDatePickerLanguage" style="visibility: hidden; width: 0px"
								type="month" ref="datePicker" @change="datePickerDatachange()"
								v-model="value_monthPicked"></date-picker>
					...
					<div id="monthlyAttendanceReportForm">
						<vxe-table :data="value_dataItemsToShow" stripe align="center" :cell-style="cellStyle"
							...
							<vxe-table-column :show-overflow="ellipsisMode" field="person" :title="disp_person"
								width="10%" align="left" type="html" style="font: 10px"></vxe-table-column>
							...
							<vxe-table-column :show-overflow="ellipsisMode" field="workingTimeToShow"
								:title="disp_working_time" align="left" type="html" width="20%"></vxe-table-column>
							<vxe-table-column :show-overflow="ellipsisMode" field="attendanceStatusText"
								:title="disp_attendanceStatus" align="left" type="html" width="33%"></vxe-table-column>
							<vxe-table-column field="attendanceStatus" :title="disp_monthlyAttendanceStatus"
								min-width="400" type="html" />
		...
		methods: {
			...
			exportMaterToExcel() {
				...
				for (let idx = 0; idx < self.value_allTableItems.length; idx++) {
					...
					for (
						let j = 0;
						j < item.attendanceStatusData.attendance_data.working_time.length;
						j++
					) {
						if (item.attendanceStatusData.attendance_data.working_time[j] > 0) {
							obj["D" + (j + 1)] = self.formatDurationTime(
								item.attendanceStatusData.attendance_data.working_time[j]
							);
				...
				for (let idx = 0; idx < self.value_allTableItems.length; idx++) {
					...
					for (
						let j = 0;
						j < item.attendanceStatusData.attendance_data.over_time.length;
						j++
					) {
						if (item.attendanceStatusData.attendance_data.over_time[j] > 0) {
							obj["D" + (j + 1)] = self.formatDurationTime(
								item.attendanceStatusData.attendance_data.over_time[j]
							);
				...
				for (let idx = 0; idx < self.value_allTableItems.length; idx++) {
					...
					for (
						let j = 0;
						j < item.attendanceStatusData.attendance_data.clock_in_status.length;
						j++
					) {
						if (
							item.attendanceStatusData.attendance_data.clock_in_status[j] == 5 &&
				...
				for (let idx = 0; idx < self.value_allTableItems.length; idx++) {
					...
					for (
						let j = 0;
						j < item.attendanceStatusData.attendance_data.clock_out_status.length;
						j++
					) {
				...
				if (workbook != null) {
					workbook.xlsx.writeBuffer().then((data) => {
						...
						var filename = `Monthly_Attendance_${self.value_specifiedMonth
							.yyyy_mm()
							.replace("-", "")}.xlsx`;
						...
						zip.generateAsync({ type: "blob" }).then(function (content) {
							FileSaver.saveAs(
								content,
								`Monthly_Attendance_${self.value_specifiedMonth
									.yyyy_mm()
									.replace("-", "")}.zip`
							);
						});
			...
			exportMasterToTXTCSV() {
				...
				if (self.value_fileType == ".txt") {
					filename = `Monthly_Attendance_${self.value_specifiedMonth.yyyy_mm().replace("-", "")}.txt`
					blob = new Blob([data], {
						type: "text/plain;charset=utf-8"
					});
				}
				else if (self.value_fileType == ".csv") {
					filename = `Monthly_Attendance_${self.value_specifiedMonth.yyyy_mm().replace("-", "")}.csv`
					blob = new Blob([data], {
			...
			async exportDetailToExcel() {
				...
				for (let idx = 0; idx < self.value_attendanceDataListToReview.length; idx++) {
...

					{
						...
						switch (self.value_attendanceDataListToReview[idx].verify_mode) {
							case 1:
								self.value_attendanceDataListToReview[idx][
									"clockMode"
								] = i18n.formatter.format("ClockModeCard");
								break;
							case 2:
								self.value_attendanceDataListToReview[idx][
									"clockMode"
								] = i18n.formatter.format("ClockModePass");
								break;
							case 3:
								self.value_attendanceDataListToReview[idx][
									"clockMode"
								] = i18n.formatter.format("ClockModeClockIn");
								break;
							case 4:
								self.value_attendanceDataListToReview[idx][
									"clockMode"
								] = i18n.formatter.format("ClockModeClockOut");
								break;
							default:
								self.value_attendanceDataListToReview[idx][
									"clockMode"
								] = i18n.formatter.format("None");
								break;
						}
				...
				if (workbook != null) {
					workbook.xlsx.writeBuffer().then((data) => {
						var filename = `Attendance_${self.value_specifiedMonth
							.yyyy_mm()
						...
						zip.generateAsync({ type: "blob" }).then(function (content) {
							FileSaver.saveAs(
								content,
								`Attendance_${self.value_specifiedMonth
									.yyyy_mm()
									.replace("-", "")}_${personName}.zip`
							);
						});
					});
				}
			...
			generateFilteredData(sourceData, filter) {
				...
				filteredItems.forEach((item) => {
					...
					for (
						let i = 0;
						i < item["attendanceStatusData"].attendance_data.working_time.length;
						i++
					) {
						totalWorkingTime +=
					...
					for (
						let i = 0;
						i < item["attendanceStatusData"].attendance_data.over_time.length;
						i++
					) {
					...
					if (totalWorkingTime > 0)
						item["workingTimeToShow"] +=
							i18n.formatter.format("WorkingTime") +
							" : " +
							self.formatDurationTime(totalWorkingTime);

					if (totalOverTime > 0)
						item["workingTimeToShow"] +=
							"<br>" +
							i18n.formatter.format("Overtime") +
							" : " +
							self.formatDurationTime(totalOverTime);
					// const detailsButtonId = "actionOnDetails_" + item.uuid;
					// item['details'] = [
					//   `<div align="left">`,
					...
					item["attendanceStatusText"] =
						...
						item.no_record +
						"<br>" +
						i18n.formatter.format("LatePerson") +
						" : " +
						item.late +
						"<br>" +
						...
						item.early +
						"<br>" +
						i18n.formatter.format("NoEntryRecord") +
						" : " +
						item.no_entry +
						"<br>" +
						i18n.formatter.format("NoLeaveRecord") +
						" : " +
						item.no_leave +
						"<br>" +
						i18n.formatter.format("OvertimeDays") +
						" : " +
						item.overtime +
						"<br>";
				});
			...
			generateAttendanceStatusData_V2(workingHourSettings, item) {
				...
				for (let i = 0; i < daysOnMonth; i++) {
					...
					if (attRecList) {
						...
						if (timeToCheckClockOut && clockOutStatus == _STATUS_NONE) {
							...
							if (passModeRecord.length > 0) {
								...
								if (defined_overtime_enabled) {
									if (
										lastClockOutRec.timestamp >=
										tDayStartTs + overTime + overTimeBuffer
									) {
										// over time
										// clockOutStatus = _STATUS_OVER_TIME;
						...
						if (
							(!holiday && clockInStatus == _STATUS_NO_SHOW) ||
						...
						returnData.attendance_data.over_time_record.push({
							overTimeStartStamp,
							overTimeEndStamp,
						});
