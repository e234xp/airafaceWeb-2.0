<template>
  <div>
    <CCol sm="12">
      <div class="h1">{{ disp_header }}</div>

      <CCard>
        <CCardBody>
          <CRow class="align-items-center">
            <CButton class="btn btn-primary btn-w-normal mx-3" @click="clickOnApplySettings" :disabled="
                value_useHttp_disp
                  ? !(flag_dataChanged && value_httpParamPass)
                  : !(value_useHttp_disp != value_useHttp || flag_dataChanged)
              ">
              <div style="font-size: 20px">{{ disp_apply }}</div>
            </CButton>

            <CSwitch size="lg" color="secondary" :checked="value_useHttp_disp" @update:checked="switchMode()" />

            <div class="h2 mb-0 ml-3 p-0">{{ disp_settingType }}</div>
          </CRow>
        </CCardBody>
      </CCard>
      <CCard v-if="!value_useHttp_disp">
        <CCardBody>
          <CRow>
            <!-- 辨識成功提示 左欄 -->
            <CCol sm="3">
              <CCol sm="12">
                <div class="h4 p-0 mt-3 h-xxxl d-flex justify-content-center text-center">
                  {{ disp_headerVerifiedIndication }}
                </div>

                <!-- 綠色區 -->
                <div class="w-100 d-flex flex-column align-items-center"
                  :style="`background-color:${value_verifiedBackgroundColor_disp};`">
                  <div class="fz-lg h-xxxl font-weight-bold text-center p-2 mb-3 text-break"
                    :style="`color:${value_verifiedTextColor_disp};`">
                    {{ value_verifiedTopMessage_converted || value_topMessage_default }}
                  </div>

                  <img class="w-100 p-3 mb-3" v-if="!value_verifiedImageUseDynamicQrCode_disp" id="verified_image" :src="
                      value_verifiedIndicationImage_disp ||
                      value_verifiedIndicationImage_default
                    " />

                  <vue-qr v-if="value_verifiedImageUseDynamicQrCode_disp"
                    :text="value_verifiedImageQrcodeString_converted" class="w-75 mb-3"></vue-qr>

                  <div class="fz-lg h-xxxl font-weight-bold text-center"
                    :style="`color:${value_verifiedTextColor_disp};`">
                    {{
                    value_verifiedBottomMessage_converted || value_bottomMessage_default
                    }}
                  </div>

                  <div v-if="value_verifiedImageUseDynamicQrCode_disp">
                    <textarea class="form-control mb-3" rows="10" :value="value_dynamicParamsFormat"
                      readonly></textarea>
                  </div>
                </div>
              </CCol>
            </CCol>
            <!-- 辨識成功提示 右欄 -->
            <CCol sm="3">
              <CCol sm="12">
                <CCol sm="12">
                  <div class="h5 mt-3">{{ disp_topMessage }}</div>
                  <CInput size="lg" v-model="value_verifiedTopMessage_disp" :placeholder="disp_defaultValue" />
                </CCol>
                <CCol sm="12">
                  <div class="h5">{{ disp_bottomMessage }}</div>
                  <CInput size="lg" v-model="value_verifiedBottomMessage_disp" :placeholder="disp_defaultValue" />
                </CCol>
                <CCol sm="12">
                  <div class="h5">{{ disp_textColor }}</div>
                  <CInput size="lg" type="color" style="" v-model="value_verifiedTextColor_disp" />
                </CCol>
                <CCol sm="12">
                  <div class="h5">{{ disp_backgroundColor }}</div>
                  <CInput size="lg" type="color" style="" v-model="value_verifiedBackgroundColor_disp" />
                </CCol>
                <CCol sm="12">
                  <div class="h5">{{ disp_displayImage }}</div>
                  <div style="height: 10px" />
                  <CRow>
                    <CSwitch color="secondary" style="margin: 0 20px 0 15px"
                      :checked="value_verifiedImageUseDynamicQrCode_disp" @update:checked="switchVerifiedImageType()" />
                    <div class="h5">{{ disp_displayVerifiedImageType }}</div>
                  </CRow>
                  <div style="height: 20px" />
                  <CButton v-if="!value_verifiedImageUseDynamicQrCode_disp" variant="outline"
                    style="height: 45px; color: #20a8d8; border: 1px solid #20a8d8" size="lg" color="light"
                    @click="clickOnPickImageForVerified">{{ disp_selectImage }}
                  </CButton>
                  <textarea v-if="value_verifiedImageUseDynamicQrCode_disp" class="form-control"
                    style="font-size: 14px; resize: none" rows="7"
                    v-model="value_verifiedImageQrcodeString_disp"></textarea>
                </CCol>
              </CCol>
            </CCol>

            <!-- 辨識失敗提示 左欄 = -->
            <CCol sm="3">
              <CCol sm="12">
                <div class="h4 p-0 mt-3 h-xxxl d-flex justify-content-center text-center">
                  {{ disp_headerUnverifiedIndication }}
                </div>

                <!-- 紅色區 -->
                <div class="w-100 d-flex flex-column align-items-center"
                  :style="`background-color:${value_unverifiedBackgroundColor_disp};`">
                  <div class="fz-lg h-xxxl font-weight-bold text-center p-2 mb-3 text-break"
                    :style="`color:${value_unverifiedTextColor_disp};`">
                    {{ value_unverifiedTopMessage_converted || value_topMessage_default }}
                  </div>

                  <img v-if="!value_unverifiedImageUseDynamicQrCode_disp" class="w-100 p-3 mb-3" id="nonVerified_image"
                    :src="
                      value_unverifiedIndicationImage_disp ||
                      value_unverifiedIndicationImage_default
                    " />
                  <vue-qr v-if="value_unverifiedImageUseDynamicQrCode_disp"
                    :text="value_unverifiedImageQrcodeString_converted" class="w-75 mb-3"></vue-qr>

                  <div class="fz-lg h-xxxl font-weight-bold text-center"
                    :style="`color:${value_unverifiedTextColor_disp};`">
                    {{
                    value_unverifiedBottomMessage_converted ||
                    value_bottomMessage_default
                    }}
                  </div>

                  <div v-if="value_unverifiedImageUseDynamicQrCode_disp">
                    <textarea class="form-control mb-3" rows="10" :value="value_dynamicParamsFormat"
                      readonly></textarea>
                  </div>
                </div>
              </CCol>
            </CCol>
            <!-- 辨識失敗提示 右欄 = -->
            <CCol sm="3">
              <CCol sm="12">
                <CCol sm="12">
                  <div class="h5 mt-3">{{ disp_topMessage }}</div>
                  <CInput size="lg" v-model="value_unverifiedTopMessage_disp" :placeholder="disp_defaultValue" />
                </CCol>
                <CCol sm="12">
                  <div class="h5">{{ disp_bottomMessage }}</div>
                  <CInput size="lg" v-model="value_unverifiedBottomMessage_disp" :placeholder="disp_defaultValue" />
                </CCol>
                <CCol sm="12">
                  <div class="h5">{{ disp_textColor }}</div>
                  <CInput size="lg" type="color" style="" v-model="value_unverifiedTextColor_disp" />
                </CCol>
                <CCol sm="12">
                  <div class="h5">{{ disp_backgroundColor }}</div>
                  <CInput size="lg" type="color" style="" v-model="value_unverifiedBackgroundColor_disp" />
                </CCol>
                <CCol sm="12">
                  <div class="h5">{{ disp_displayImage }}</div>
                  <div style="height: 10px" />
                  <CRow>
                    <CSwitch color="secondary" style="margin: 0 20px 0 15px"
                      :checked="value_unverifiedImageUseDynamicQrCode_disp"
                      @update:checked="switchNonVerifiedImageType()" />
                    <div class="h5">{{ disp_displayUnverifiedImageType }}</div>
                  </CRow>
                  <div style="height: 20px" />
                  <CButton v-if="!value_unverifiedImageUseDynamicQrCode_disp" variant="outline"
                    style="height: 45px; color: #20a8d8; border: 1px solid #20a8d8" size="lg" color="light"
                    @click="clickOnPickImageForUnverified">{{ disp_selectImage }}
                  </CButton>
                  <textarea v-if="value_unverifiedImageUseDynamicQrCode_disp" class="form-control"
                    style="font-size: 14px; resize: none" rows="7"
                    v-model="value_unverifiedImageQrcodeString_disp"></textarea>
                </CCol>
              </CCol>
            </CCol>
          </CRow>
        </CCardBody>
        <CRow>
          <CCol sm="12">
            <div class="row justify-content-center">
              <CButton @click="clickOnDefaultSettings" class="btn btn-primary mb-3" size="lg">
                {{ disp_defaultValue }}
              </CButton>
            </div>
          </CCol>
        </CRow>
      </CCard>

      <CCard v-if="value_useHttp_disp">
        <CCardBody>
          <CCol sm="12">
            <CRow>
              <CCol sm="3">
                <div class="column-space-between">
                  <div class="h5">{{ disp_httpHostAddress }}</div>
                  <CInput size="lg" :invalid-feedback="disp_noEmptyNoSpaceOnly" value=""
                    v-model="value_httpHostAddress_disp" placeholder="" :is-valid="hostAddressValidator" required />
                </div>
              </CCol>
              <CCol sm="3">
                <div class="column-space-between">
                  <div class="h5">{{ disp_httpEnabledSSL }}</div>
                  <CSwitch size="lg" class="ml-0 mb-16" color="success" shape="pill" @update:checked="
                      value_httpEnabledSSL_disp = !value_httpEnabledSSL_disp
                    " :checked="value_httpEnabledSSL_disp">
                  </CSwitch>
                </div>
              </CCol>
              <CCol sm="3">
                <div class="column-space-between">
                  <div class="h5">{{ disp_httpUser }}</div>
                  <CInput size="lg" value="" v-model="value_httpUser_disp" placeholder="" />
                </div>
              </CCol>
              <CCol sm="3">
                <div class="column-space-between">
                  <div class="h5">{{ disp_httpPass }}</div>
                  <form>
                    <CInput size="lg" value="" v-model="value_httpPass_disp" placeholder=""
                      :type="flag_view_password ? 'text' : 'password'" autocomplete="on">
                      <template #append-content>
                        <CButton @click="viewPassword" style="padding: 0.375rem 0.375rem;">
                          <CIcon v-show="flag_view_password" src="/img/eye-slash.png" />
                          <CIcon v-show="!flag_view_password" src="/img/eye.png" />
                        </CButton>
                      </template>
                    </CInput>
                  </form>
                </div>
              </CCol>
            </CRow>
            <CRow>
              <CCol sm="3">
                <div class="column-space-between">
                  <div class="h5">{{ disp_httpPort }}</div>
                  <CInput size="lg" value="" v-model="value_httpPort_disp" placeholder="" :is-valid="numberValidator"
                    :invalid-feedback="disp_numbersOnly" />
                </div>
              </CCol>
              <CCol sm="6">
                <div class="column-space-between">
                  <div class="h5">
                    {{ disp_httpUrl }}
                  </div>
                  <CInput size="lg" value="" v-model="value_httpUrl_disp" placeholder="" :is-valid="urlValidator"
                    :invalid-feedback="disp_badFormat" />
                </div>
              </CCol>
              <CCol sm="3">
                <div class="column-space-between">
                  <div class="h5">{{ disp_connectTimeoutMs }}</div>
                  <CInput size="lg" value="" v-model="value_httpTimeout_disp" placeholder="" :is-valid="numberValidator"
                    :invalid-feedback="disp_numbersOnly" />
                </div>
              </CCol>
            </CRow>
            <CRow>
              <CCol sm="6">
                <div class="h5">REQUEST URL</div>
                <textarea class="form-control" style="resize: none; font-size: 20px" rows="9" :value="value_completeUrl"
                  readonly></textarea>
              </CCol>
              <CCol sm="6">
                <div class="h5">RESPONSE ( Content-Type : "application/json" )</div>
                <textarea class="form-control" style="resize: none; font-size: 20px" rows="9"
                  :value="value_resposeFormat" readonly></textarea>
              </CCol>
            </CRow>
          </CCol>
        </CCardBody>
      </CCard>
    </CCol>
  </div>
</template>

<script>
  import i18n from "../../i18n";
  import VueQr from "vue-qr";

  const verifiedIndicationImage_default =
    "data:image/png;base64,";
  const unverifiedIndicationImage_default =
    "data:image/png;base64,";

  export default {
    name: "FactoryDefault",

    data() {
      return {
        param_cardStyle: "height: 35rem;",

        flag_view_password: false,

        disp_header: i18n.formatter.format("IndicationSettings"),
        disp_headerVerifiedIndication: i18n.formatter.format("VerifiedIndication"),
        disp_headerUnverifiedIndication: i18n.formatter.format("UnverifiedIndication"),
        disp_apply: i18n.formatter.format("Apply"),
        disp_topMessage: i18n.formatter.format("TopMessage"),
        disp_bottomMessage: i18n.formatter.format("BottomMessage"),
        disp_textColor: i18n.formatter.format("TextColor"),
        disp_backgroundColor: i18n.formatter.format("BackgroundColor"),
        disp_displayImage: i18n.formatter.format("DisplayImage"),
        disp_selectImage: i18n.formatter.format("Select"),

        disp_displayVerifiedImageType: i18n.formatter.format("StaticPicture"),
        disp_displayUnverifiedImageType: i18n.formatter.format("StaticPicture"),

        disp_httpHostAddress: i18n.formatter.format("HostAddress"), //
        disp_httpEnabledSSL: i18n.formatter.format("HttpEnabledSSL"), //
        disp_httpUser: i18n.formatter.format("Username"), //
        disp_httpPass: i18n.formatter.format("Password"), //
        disp_httpPort: i18n.formatter.format("Port"), //
        disp_httpUrl: "PATH", //i18n.formatter.format("HttpUrl"), //
        disp_showCompleteUrl: "QUERY", //i18n.formatter.format("ShowCompleteUrl"), //

        disp_defaultValue: i18n.formatter.format("DefaultValue"),
        disp_noEmptyNorSpaceNeigherRepeat: i18n.formatter.format(
          "NoEmptyNorSpaceNeigherRepeat"
        ), //
        disp_noEmptyNoSpaceOnly: i18n.formatter.format("NoEmptyNoSpaceOnly"), //
        disp_fetchViaHttp: i18n.formatter.format("FetchViaHttp"), //
        disp_staticSettings: i18n.formatter.format("StaticSettings"), //
        disp_settingType: i18n.formatter.format("StaticSettings"), //
        disp_numbersOnly: i18n.formatter.format("NumbersOnly"),
        disp_badFormat: i18n.formatter.format("InvalidURL"),
        disp_connectTimeoutMs: i18n.formatter.format("ConnectTimetoutMs"),

        value_useHttp: false,
        value_httpHostAddress: "", //
        value_httpEnabledSSL: false, //
        value_httpUser: "", //
        value_httpPass: "", //
        value_httpPort: 80, //
        value_httpUrl: "/", //
        value_httpMethod: "GET",
        value_httpTimeout: 3000,

        value_verifiedImageUseDynamicQrCode_disp: false,
        value_verifiedImageUseDynamicQrCode: false,
        value_verifiedImageQrcodeString_disp: "",
        value_verifiedImageQrcodeString_converted: "",
        value_verifiedImageQrcodeString: "",

        value_unverifiedImageUseDynamicQrCode_disp: false,
        value_unverifiedImageUseDynamicQrCode: false,
        value_unverifiedImageQrcodeString_disp: "",
        value_unverifiedImageQrcodeString_converted: "",
        value_unverifiedImageQrcodeString: "",

        value_useHttp_disp: false,
        value_httpHostAddress_disp: "", //
        value_httpEnabledSSL_disp: false, //
        value_httpUser_disp: "", //
        value_httpPass_disp: "", //
        value_httpPort_disp: 80, //
        value_httpUrl_disp: "/", //
        value_httpMethod_disp: "GET",
        value_httpTimeout_disp: 3000,

        value_completeUrl: "",

        value_verifiedTextColor: "#ffffff",
        value_verifiedBackgroundColor: "#048729",
        value_verifiedTopMessage: "",
        value_verifiedBottomMessage: "",
        value_verifiedIndicationImage: "",

        value_verifiedTextColor_disp: "#ffffff",
        value_verifiedBackgroundColor_disp: "#048729",
        value_verifiedTopMessage_disp: "",
        value_verifiedTopMessage_converted: "",
        value_verifiedBottomMessage_disp: "",
        value_verifiedBottomMessage_converted: "",
        value_verifiedIndicationImage_disp: "",

        value_unverifiedTextColor: "#ffffff",
        value_unverifiedBackgroundColor: "#d50000",
        value_unverifiedTopMessage: "",
        value_unverifiedBottomMessage: "",
        value_unverifiedIndicationImage: "",

        value_unverifiedTextColor_disp: "#ffffff",
        value_unverifiedBackgroundColor_disp: "#d50000",
        value_unverifiedTopMessage_disp: "",
        value_unverifiedTopMessage_converted: "",
        value_unverifiedBottomMessage_disp: "",
        value_unverifiedBottomMessage_converted: "",
        value_unverifiedIndicationImage_disp: "",

        value_topMessage_default: "",
        value_bottomMessage_default: i18n.formatter.format("DefaultValue"),
        value_verifiedIndicationImage_default: verifiedIndicationImage_default,
        value_unverifiedIndicationImage_default: unverifiedIndicationImage_default,
        value_tabletSettings: null,
        flag_changingSettings: false,
        flag_dataChanged: false,
        value_httpParamPass: false,
        value_resposeFormat: `{
  "top_message" : "message string",
  "bottom_message" : "message string",
  "text_color" : "#ffffff",
  "background_color" : "#d50000",
  "base64_image" : "base64image string"
}`,
        value_dynamicParamsFormat: `Match : {{match}}
Timestamp : {{timestamp}}
Person id : {{personId}}
Person name : {{personName}}
Group List : {{groupList}}
Temperature : {{temperature}}

http ex : https://action.go/verify?known={{match}}&id={{personId}}

sms ex : SMSTO:0988123456:{{personName}} is clockin
    `,
      };
    },
    beforeRouteEnter(to, from, next) {
      next();
    },
    beforeRouteLeave(to, from, next) {
      next();
    },
    created() {
      const self = this;
      self.flag_changingSettings = true;
      self.$globalGetTabletSetting(function (err, data) {
        if (!err) {
          self.value_tabletSettings = data;
          self.resetSettings(data);
          if (self.value_useHttp_disp) {
            self.disp_settingType = self.disp_fetchViaHttp;
          } else {
            self.disp_settingType = self.disp_staticSettings;
          }

          if (self.value_verifiedImageUseDynamicQrCode_disp) {
            self.disp_displayVerifiedImageType = i18n.formatter.format("GeneratedQrcode");
          } else {
            self.disp_displayVerifiedImageType = i18n.formatter.format("StaticPicture");
          }
          if (self.value_unverifiedImageUseDynamicQrCode_disp) {
            self.disp_displayUnverifiedImageType = i18n.formatter.format("GeneratedQrcode");
          } else {
            self.disp_displayUnverifiedImageType = i18n.formatter.format("StaticPicture");
          }
          self.buildUrl();
        }
        self.flag_changingSettings = false;
      });
    },
    watch: {
      value_useHttp_disp: function (value) {
        const self = this;
        if (value) {
          self.disp_settingType = self.disp_fetchViaHttp;
        } else {
          self.disp_settingType = self.disp_staticSettings;
        }
      },
      value_httpHostAddress_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
        this.buildUrl();
      },
      value_httpEnabledSSL_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
        this.buildUrl();
      },
      value_httpUser_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
        this.buildUrl();
      },
      value_httpPass_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
        this.buildUrl();
      },
      value_httpPort_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
        this.buildUrl();
      },
      value_httpUrl_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
        this.buildUrl();
      },
      value_httpTimeout_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
        this.buildUrl();
      },
      value_verifiedTextColor_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
      },
      value_verifiedBackgroundColor_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
      },
      value_verifiedTopMessage_disp: function (value) {
        this.value_verifiedTopMessage_converted = this.covertString(value);
        this.flag_dataChanged = this.dataChanged();
      },
      value_verifiedBottomMessage_disp: function (value) {
        this.value_verifiedBottomMessage_converted = this.covertString(value);
        this.flag_dataChanged = this.dataChanged();
      },
      value_verifiedIndicationImage_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
      },
      value_unverifiedTextColor_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
      },
      value_unverifiedBackgroundColor_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
      },
      value_unverifiedTopMessage_disp: function (value) {
        this.value_unverifiedTopMessage_converted = this.covertString(value);

        this.flag_dataChanged = this.dataChanged();
      },
      value_unverifiedBottomMessage_disp: function (value) {
        this.value_unverifiedBottomMessage_converted = this.covertString(value);
        this.flag_dataChanged = this.dataChanged();
      },
      value_unverifiedIndicationImage_disp: function (value) {
        this.flag_dataChanged = this.dataChanged();
      },
      value_verifiedImageQrcodeString_disp: function (value) {
        this.value_verifiedImageQrcodeString_converted = this.covertStringQr(value);
        this.flag_dataChanged = this.dataChanged();
      },
      value_unverifiedImageQrcodeString_disp: function (value) {
        this.value_unverifiedImageQrcodeString_converted = this.covertStringQr(value);
        this.flag_dataChanged = this.dataChanged();
      },
    },
    components: { VueQr },
    methods: {
      viewPassword() {
        const self = this;

        self.flag_view_password = !self.flag_view_password;
      },

      covertString(value) {
        var s = value;
        const formatDate = (fmt) => {
          var d = new Date();
          var o = {
            "M+": d.getMonth() + 1, //月份
            "d+": d.getDate(), //日
            "h+": d.getHours(), //小時
            "m+": d.getMinutes(), //分
            "s+": d.getSeconds(), //秒
            "q+": Math.floor((d.getMonth() + 3) / 3), //季度
            S: d.getMilliseconds(), //毫秒
          };
          if (/(y+)/.test(fmt))
            fmt = fmt.replace(
              RegExp.$1,
              (d.getFullYear() + "").substr(4 - RegExp.$1.length)
            );
          for (var k in o)
            if (new RegExp("(" + k + ")").test(fmt))
              fmt = fmt.replace(
                RegExp.$1,
                RegExp.$1.length == 1 ? o[k] : ("00" + o[k]).substr(("" + o[k]).length)
              );
          return fmt;
        };
        s = s.replaceAll("{{personId}}", "A-001");
        s = s.replaceAll("{{personName}}", "John");
        s = s.replaceAll("{{match}}", "true");
        s = s.replaceAll("{{timestamp}}", Date.now());
        s = s.replaceAll("{{groupList}}", '["vip"]');
        s = s.replaceAll("{{temperature}}", "36.5");
        s = s.replaceAll("{{date}}", formatDate("yyyy-MM-dd"));
        s = s.replaceAll("{{time}}", formatDate("hh:mm:ss"));
        s = s.replaceAll("{{br}}", "\n");
        return s;
      },
      covertStringQr(value) {
        var s = value;
        s = s.replaceAll("{{personId}}", "A-001");
        s = s.replaceAll("{{personName}}", "John");
        s = s.replaceAll("{{match}}", "true");
        s = s.replaceAll("{{timestamp}}", "1234567890123");
        s = s.replaceAll("{{groupList}}", '["vip"]');
        s = s.replaceAll("{{temperature}}", "36.5");
        return s;
      },
      switchMode() {
        const self = this;
        self.value_useHttp_disp = !self.value_useHttp_disp;
        self.value_verifiedTextColor_disp = self.value_verifiedTextColor;
        self.value_verifiedBackgroundColor_disp = self.value_verifiedBackgroundColor;
        self.value_verifiedTopMessage_disp = self.value_verifiedTopMessage;
        self.value_verifiedBottomMessage_disp = self.value_verifiedBottomMessage;
        self.value_verifiedIndicationImage_disp = self.value_verifiedIndicationImage;

        self.value_unverifiedTextColor_disp = self.value_unverifiedTextColor;
        self.value_unverifiedBackgroundColor_disp = self.value_unverifiedBackgroundColor;
        self.value_unverifiedTopMessage_disp = self.value_unverifiedTopMessage;
        self.value_unverifiedBottomMessage_disp = self.value_unverifiedBottomMessage;
        self.value_unverifiedIndicationImage_disp = self.value_unverifiedIndicationImage;

        self.value_httpHostAddress_disp = self.value_httpHostAddress;
        self.value_httpEnabledSSL_disp = self.value_httpEnabledSSL;
        self.value_httpUser_disp = self.value_httpUser;
        self.value_httpPass_disp = self.value_httpPass;
        self.value_httpPort_disp = self.value_httpPort;
        self.value_httpUrl_disp = self.value_httpUrl;
        self.value_httpMethod_disp = self.value_httpMethod;
        self.value_httpTimeout_disp = self.value_httpTimeout;

        self.value_verifiedImageUseDynamicQrCode_disp =
          self.value_verifiedImageUseDynamicQrCode;
        self.value_verifiedImageQrcodeString_disp = self.value_verifiedImageQrcodeString;
        if (self.value_verifiedImageUseDynamicQrCode_disp) {
          self.disp_displayVerifiedImageType = i18n.formatter.format("GeneratedQrcode");
        } else {
          self.disp_displayVerifiedImageType = i18n.formatter.format("StaticPicture");
        }

        self.value_unverifiedImageUseDynamicQrCode_disp =
          self.value_unverifiedImageUseDynamicQrCode;
        self.value_unverifiedImageQrcodeString_disp =
          self.value_unverifiedImageQrcodeString;
        if (self.value_unverifiedImageUseDynamicQrCode_disp) {
          self.disp_displayUnverifiedImageType = i18n.formatter.format("GeneratedQrcode");
        } else {
          self.disp_displayUnverifiedImageType = i18n.formatter.format("StaticPicture");
        }
        //self.buildUrl();
      },
      switchVerifiedImageType() {
        const self = this;
        self.value_verifiedImageUseDynamicQrCode_disp = !self.value_verifiedImageUseDynamicQrCode_disp;
        self.value_verifiedImageQrcodeString_disp = self.value_verifiedImageQrcodeString;

        if (self.value_verifiedImageUseDynamicQrCode_disp) {
          self.disp_displayVerifiedImageType = i18n.formatter.format("GeneratedQrcode");
        } else {
          self.disp_displayVerifiedImageType = i18n.formatter.format("StaticPicture");
        }
        self.flag_dataChanged = self.dataChanged();
      },
      switchNonVerifiedImageType() {
        const self = this;
        self.value_unverifiedImageUseDynamicQrCode_disp = !self.value_unverifiedImageUseDynamicQrCode_disp;
        self.value_unverifiedImageQrcodeString_disp =
          self.value_unverifiedImageQrcodeString;

        if (self.value_unverifiedImageUseDynamicQrCode_disp) {
          self.disp_displayUnverifiedImageType = i18n.formatter.format("GeneratedQrcode");
        } else {
          self.disp_displayUnverifiedImageType = i18n.formatter.format("StaticPicture");
        }
        self.flag_dataChanged = self.dataChanged();
      },
      dataChanged() {
        const self = this;
        var changed =
          self.value_verifiedTextColor_disp != self.value_verifiedTextColor ||
          self.value_verifiedBackgroundColor_disp != self.value_verifiedBackgroundColor ||
          self.value_verifiedTopMessage_disp != self.value_verifiedTopMessage ||
          self.value_verifiedBottomMessage_disp != self.value_verifiedBottomMessage ||
          self.value_verifiedIndicationImage_disp != self.value_verifiedIndicationImage ||
          self.value_unverifiedTextColor_disp != self.value_unverifiedTextColor ||
          self.value_unverifiedBackgroundColor_disp !=
          self.value_unverifiedBackgroundColor ||
          self.value_unverifiedTopMessage_disp != self.value_unverifiedTopMessage ||
          self.value_unverifiedBottomMessage_disp != self.value_unverifiedBottomMessage ||
          self.value_unverifiedIndicationImage_disp !=
          self.value_unverifiedIndicationImage ||
          self.value_httpHostAddress_disp != self.value_httpHostAddress ||
          self.value_httpPort_disp != self.value_httpPort ||
          self.value_httpUser_disp != self.value_httpUser ||
          self.value_httpPass_disp != self.value_httpPass ||
          self.value_httpUrl_disp != self.value_httpUrl ||
          self.value_httpMethod_disp != self.value_httpMethod ||
          self.value_httpTimeout_disp != self.value_httpTimeout ||
          self.value_verifiedImageUseDynamicQrCode_disp !=
          self.value_verifiedImageUseDynamicQrCode ||
          self.value_unverifiedImageUseDynamicQrCode_disp !=
          self.value_unverifiedImageUseDynamicQrCode ||
          self.value_verifiedImageQrcodeString_disp !=
          self.value_verifiedImageQrcodeString ||
          self.value_unverifiedImageQrcodeString_disp !=
          self.value_unverifiedImageQrcodeString;
        return changed;
      },
      buildUrl() {
        const self = this;
        self.value_completeUrl = self.value_httpEnabledSSL_disp ? "https://" : "http://";
        // if( self.value_httpUser.length > 0 ) {
        //   self.value_completeUrl += self.value_httpUser + ":" + "[password]@";
        // }
        self.value_completeUrl +=
          self.value_httpHostAddress_disp + ":" + self.value_httpPort_disp;
        if (self.value_httpUrl_disp.length == 0 || self.value_httpUrl_disp[0] != "/") {
          self.value_completeUrl += "/";
        }
        self.value_completeUrl +=
          self.value_httpUrl_disp +
          "?match=[boolean]&time=[timestamp]&personId=[string]&personName=[string]&groupList=[string]&temperature=[float]";
        self.value_httpParamPass =
          self.hostAddressValidator() &&
          self.portValidator() &&
          self.urlValidator() &&
          self.timeoutValidator();

        //console.log( " self.value_httpParamPass :", self.value_httpParamPass );
      },
      portValidator(val) {
        const self = this;
        var processStr = "" + self.value_httpPort_disp;
        if (typeof processStr != "string") return false; // we only process strings!
        return (
          !isNaN(processStr) && // use type coercion to parse the _entirety_ of the string (`parseFloat` alone does not do this)...
          !isNaN(parseFloat(processStr))
        ); // ...and ensure strings of whitespace fail
      },
      timeoutValidator(val) {
        const self = this;
        var processStr = "" + self.value_httpTimeout_disp;
        if (typeof processStr != "string") return false; // we only process strings!
        return (
          !isNaN(processStr) && // use type coercion to parse the _entirety_ of the string (`parseFloat` alone does not do this)...
          !isNaN(parseFloat(processStr))
        ); // ...and ensure strings of whitespace fail
      },
      numberValidator(val) {
        const self = this;
        var processStr = "" + val;
        if (typeof processStr != "string") return false; // we only process strings!
        return (
          !isNaN(processStr) && // use type coercion to parse the _entirety_ of the string (`parseFloat` alone does not do this)...
          !isNaN(parseFloat(processStr))
        ); // ...and ensure strings of whitespace fail
      },
      urlValidatorFn(val) {
        var pattern = new RegExp(
          "^(https?:\\/\\/)?" + // protocol
          "((([a-z\\d]([a-z\\d-]*[a-z\\d])*)\\.)+[a-z]{2,}|" + // domain name
          "((\\d{1,3}\\.){3}\\d{1,3}))" + // OR ip (v4) address
          "(\\:\\d+)?(\\/[-a-z\\d%_.~+]*)*" + // port and path
          "(\\?[;&a-z\\d%_.~+=-]*)?" + // query string
          "(\\#[-a-z\\d_]*)?$",
          "i"
        ); // fragment locator
        return !!pattern.test(val);
      },
      urlValidator() {
        const self = this;
        var processStr =
          "https://127.0.0.1" +
          (self.value_httpUrl_disp.length > 0 && self.value_httpUrl_disp[0] != "/"
            ? "/"
            : "") +
          self.value_httpUrl_disp;
        return self.urlValidatorFn(processStr);
      },
      hostAddressValidator() {
        let flag_pass = false;
        const self = this;
        var val = self.value_httpHostAddress_disp;
        if (self.value_useHttp_disp == true) {
          if (val.replace(/\s/g, "").length == 0) {
            flag_pass = false;
            self.flag_hostData = false;
          } else {
            flag_pass = val.length > 0;
            self.flag_hostData = true;
          }
        } else {
          self.flag_hostData = true;
          return null;
        }
        return flag_pass;
      }, //
      resizeImageFromDataURL(box, base64Str, maxWidth, maxHeight, callBack) {
        // We create an image to receive the Data URI
        let img = new Image();
        img.src = base64Str;
        img.onload = () => {
          try {
            let canvas = document.createElement("canvas");
            canvas.x = 0;
            canvas.y = 0;
            canvas.width = maxWidth;
            canvas.height = maxHeight;
            let ctx = canvas.getContext("2d");
            if (box)
              ctx.drawImage(
                img,
                box.x,
                box.y,
                box.width,
                box.height,
                0,
                0,
                maxWidth,
                maxHeight
              );
            else ctx.drawImage(img, 0, 0, img.width, img.height, 0, 0, maxWidth, maxHeight);
            if (callBack) callBack(canvas.toDataURL("image/png", 1.0));
          } catch (e) {
            if (callBack) callBack(null);
          }
        };
      },
      selectPhotoFromFile(cb) {
        const input = document.createElement("input");
        input.type = "file";
        input.accept = "image/*";
        input.onclick = function (e) {
          if (e.target.value.length == 0) {
            if (cb) cb(null);
          }
        };
        input.onchange = function (e) {
          const file = e.target.files[0];
          const reader = new FileReader();
          reader.readAsDataURL(file, "UTF-8");
          reader.onload = async (readerEvent) => {
            const img = document.createElement("img");
            img.src = readerEvent.target.result;

            if (cb) cb(img);
          };
        };
        input.click();
      },
      clickOnPickImageForVerified() {
        const self = this;
        self.selectPhotoFromFile(function (img) {
          if (img) {
            self.resizeImageFromDataURL(null, img.src, 300, 300, function (base64Img) {
              self.value_verifiedIndicationImage_disp = base64Img;
            });
          }
        });
      },
      clickOnPickImageForUnverified() {
        const self = this;
        self.selectPhotoFromFile(function (img) {
          if (img) {
            self.resizeImageFromDataURL(null, img.src, 300, 300, function (base64Img) {
              self.value_unverifiedIndicationImage_disp = base64Img;
            });
          }
        });
      },
      resetSettings(tabletSettings) {
        const self = this;
        if (tabletSettings) {
          if (tabletSettings && tabletSettings.customize_verify_indication) {
            self.value_verifiedTextColor = "#ffffff";
            self.value_verifiedBackgroundColor = "#048729";
            self.value_verifiedTopMessage = "";
            self.value_verifiedBottomMessage = "";
            self.value_verifiedIndicationImage = "";
            self.value_unverifiedTextColor = "#ffffff";
            self.value_unverifiedBackgroundColor = "#d50000";
            self.value_unverifiedTopMessage = "";
            self.value_unverifiedBottomMessage = "";
            self.value_unverifiedIndicationImage = "";

            self.value_unverifiedImageUseDynamicQrCode = false;
            self.value_unverifiedImageQrcodeString = "";
            self.value_unverifiedImageUseDynamicQrCode = false;
            self.value_unverifiedImageQrcodeString = "";

            self.value_useHttp = false;
            self.value_httpHostAddress = "";
            self.value_httpEnabledSSL = false;
            self.value_httpUser = "";
            self.value_httpPass = "";
            self.value_httpPort = 80;
            self.value_httpUrl = "/";
            self.value_httpMethod = "GET";
            self.value_httpTimeout = 3000;

            if (tabletSettings.customize_verify_indication.static_verified_info) {
              self.value_verifiedTextColor =
                tabletSettings.customize_verify_indication.static_verified_info
                  .text_color || "#ffffff";
              self.value_verifiedBackgroundColor =
                tabletSettings.customize_verify_indication.static_verified_info
                  .background_color || "#048729";
              self.value_verifiedTopMessage =
                tabletSettings.customize_verify_indication.static_verified_info
                  .top_message || "";
              self.value_verifiedBottomMessage =
                tabletSettings.customize_verify_indication.static_verified_info
                  .bottom_message || "";
              self.value_verifiedIndicationImage =
                tabletSettings.customize_verify_indication.static_verified_info
                  .base64_image || "";

              self.value_verifiedImageUseDynamicQrCode =
                tabletSettings.customize_verify_indication.static_verified_info
                  .use_qrcode || false;
              self.value_verifiedImageQrcodeString =
                tabletSettings.customize_verify_indication.static_verified_info
                  .qrcode_string || "";
            }
            if (tabletSettings.customize_verify_indication.static_unverified_info) {
              self.value_unverifiedTextColor =
                tabletSettings.customize_verify_indication.static_unverified_info
                  .text_color || "#ffffff";
              self.value_unverifiedBackgroundColor =
                tabletSettings.customize_verify_indication.static_unverified_info
                  .background_color || "#d50000";
              self.value_unverifiedTopMessage =
                tabletSettings.customize_verify_indication.static_unverified_info
                  .top_message || "";
              self.value_unverifiedBottomMessage =
                tabletSettings.customize_verify_indication.static_unverified_info
                  .bottom_message || "";
              self.value_unverifiedIndicationImage =
                tabletSettings.customize_verify_indication.static_unverified_info
                  .base64_image || "";

              self.value_unverifiedImageUseDynamicQrCode =
                tabletSettings.customize_verify_indication.static_unverified_info
                  .use_qrcode || false;
              self.value_unverifiedImageQrcodeString =
                tabletSettings.customize_verify_indication.static_unverified_info
                  .qrcode_string || "";
            }
            if (tabletSettings.customize_verify_indication.http_custom_connection_info) {
              self.value_httpHostAddress =
                tabletSettings.customize_verify_indication.http_custom_connection_info
                  .host || "";
              self.value_httpEnabledSSL =
                tabletSettings.customize_verify_indication.http_custom_connection_info
                  .use_ssl || false;
              self.value_httpUser =
                tabletSettings.customize_verify_indication.http_custom_connection_info
                  .username || "";
              self.value_httpPass =
                tabletSettings.customize_verify_indication.http_custom_connection_info
                  .password || "";
              self.value_httpPort =
                tabletSettings.customize_verify_indication.http_custom_connection_info
                  .port || 80;
              self.value_httpUrl =
                tabletSettings.customize_verify_indication.http_custom_connection_info
                  .url || "";
              self.value_httpMethod =
                tabletSettings.customize_verify_indication.http_custom_connection_info
                  .method || "GET";
              self.value_httpTimeout =
                tabletSettings.customize_verify_indication.http_custom_connection_info
                  .timeout || 3000;
            }
            if (tabletSettings.customize_verify_indication.use_http != null) {
              self.value_useHttp = tabletSettings.customize_verify_indication.use_http;
            }

            self.value_verifiedTextColor_disp = self.value_verifiedTextColor;
            self.value_verifiedBackgroundColor_disp = self.value_verifiedBackgroundColor;
            self.value_verifiedTopMessage_disp = self.value_verifiedTopMessage;
            self.value_verifiedBottomMessage_disp = self.value_verifiedBottomMessage;
            self.value_verifiedIndicationImage_disp = self.value_verifiedIndicationImage;

            self.value_unverifiedTextColor_disp = self.value_unverifiedTextColor;
            self.value_unverifiedBackgroundColor_disp =
              self.value_unverifiedBackgroundColor;
            self.value_unverifiedTopMessage_disp = self.value_unverifiedTopMessage;
            self.value_unverifiedBottomMessage_disp = self.value_unverifiedBottomMessage;
            self.value_unverifiedIndicationImage_disp =
              self.value_unverifiedIndicationImage;

            self.value_useHttp_disp = self.value_useHttp;
            self.value_httpHostAddress_disp = self.value_httpHostAddress;
            self.value_httpEnabledSSL_disp = self.value_httpEnabledSSL;
            self.value_httpUser_disp = self.value_httpUser;
            self.value_httpPass_disp = self.value_httpPass;
            self.value_httpPort_disp = self.value_httpPort;
            self.value_httpUrl_disp = self.value_httpUrl;
            self.value_httpMethod_disp = self.value_httpMethod;
            self.value_httpTimeout_disp = self.value_httpTimeout;

            self.value_verifiedImageUseDynamicQrCode_disp =
              self.value_verifiedImageUseDynamicQrCode;
            self.value_verifiedImageQrcodeString_disp =
              self.value_verifiedImageQrcodeString;
            self.value_unverifiedImageUseDynamicQrCode_disp =
              self.value_unverifiedImageUseDynamicQrCode;
            self.value_unverifiedImageQrcodeString_disp =
              self.value_unverifiedImageQrcodeString;
          }
        } else {
          self.value_verifiedTextColor_disp = "#ffffff";
          self.value_verifiedBackgroundColor_disp = "#048729";
          self.value_verifiedTopMessage_disp = "";
          self.value_verifiedBottomMessage_disp = "";
          self.value_verifiedIndicationImage_disp = "";

          self.value_unverifiedTextColor_disp = "#ffffff";
          self.value_unverifiedBackgroundColor_disp = "#d50000";
          self.value_unverifiedTopMessage_disp = "";
          self.value_unverifiedBottomMessage_disp = "";
          self.value_unverifiedIndicationImage_disp = "";

          self.value_verifiedImageUseDynamicQrCode_disp = false;
          self.value_verifiedImageQrcodeString_disp = "";
          self.value_unverifiedImageUseDynamicQrCode_disp = false;
          self.value_unverifiedImageQrcodeString_disp = "";
        }
      },
      clickOnDefaultSettings() {
        const self = this;
        self.resetSettings();
      },
      async clickOnApplySettings() {
        const self = this;
        self.flag_changingSettings = true;
        self.value_tabletSettings["customize_verify_indication"] = {
          use_http: self.value_useHttp_disp,
          static_verified_info: {},
          static_unverified_info: {},
        };

        if (self.value_useHttp_disp) {
          self.value_tabletSettings.customize_verify_indication[
            "http_custom_connection_info"
          ] = {};
          self.value_tabletSettings.customize_verify_indication.http_custom_connection_info[
            "use_ssl"
          ] = self.value_httpEnabledSSL_disp;
          self.value_tabletSettings.customize_verify_indication.http_custom_connection_info[
            "host"
          ] = self.value_httpHostAddress_disp;
          self.value_tabletSettings.customize_verify_indication.http_custom_connection_info[
            "port"
          ] = self.value_httpPort_disp;
          self.value_tabletSettings.customize_verify_indication.http_custom_connection_info[
            "url"
          ] = self.value_httpUrl_disp;
          self.value_tabletSettings.customize_verify_indication.http_custom_connection_info[
            "username"
          ] = self.value_httpUser_disp;
          self.value_tabletSettings.customize_verify_indication.http_custom_connection_info[
            "password"
          ] = self.value_httpPass_disp;
          self.value_tabletSettings.customize_verify_indication.http_custom_connection_info[
            "method"
          ] = self.value_httpMethod_disp;
          self.value_tabletSettings.customize_verify_indication.http_custom_connection_info[
            "timeout"
          ] = self.value_httpTimeout_disp;
        } else {
          if (self.value_verifiedTextColor_disp) {
            self.value_tabletSettings.customize_verify_indication.static_verified_info[
              "text_color"
            ] = self.value_verifiedTextColor_disp;
          }
          if (self.value_verifiedBackgroundColor_disp) {
            self.value_tabletSettings.customize_verify_indication.static_verified_info[
              "background_color"
            ] = self.value_verifiedBackgroundColor_disp;
          }
          if (self.value_verifiedTopMessage_disp) {
            self.value_tabletSettings.customize_verify_indication.static_verified_info[
              "top_message"
            ] = self.value_verifiedTopMessage_disp;
          }
          if (self.value_verifiedBottomMessage_disp) {
            self.value_tabletSettings.customize_verify_indication.static_verified_info[
              "bottom_message"
            ] = self.value_verifiedBottomMessage_disp;
          }

          self.value_tabletSettings.customize_verify_indication.static_verified_info[
            "use_qrcode"
          ] = self.value_verifiedImageUseDynamicQrCode_disp;
          if (self.value_verifiedImageUseDynamicQrCode_disp) {
            if (self.value_verifiedImageQrcodeString_disp) {
              self.value_tabletSettings.customize_verify_indication.static_verified_info[
                "qrcode_string"
              ] = self.value_verifiedImageQrcodeString_disp;
            } else {
              self.value_tabletSettings.customize_verify_indication.static_verified_info[
                "qrcode_string"
              ] = "";
            }
          } else {
            if (self.value_verifiedIndicationImage_disp) {
              self.value_tabletSettings.customize_verify_indication.static_verified_info[
                "base64_image"
              ] = self.value_verifiedIndicationImage_disp;
            }
          }

          if (self.value_unverifiedTextColor_disp) {
            self.value_tabletSettings.customize_verify_indication.static_unverified_info[
              "text_color"
            ] = self.value_unverifiedTextColor_disp;
          }
          if (self.value_unverifiedBackgroundColor_disp) {
            self.value_tabletSettings.customize_verify_indication.static_unverified_info[
              "background_color"
            ] = self.value_unverifiedBackgroundColor_disp;
          }
          if (self.value_unverifiedTopMessage_disp) {
            self.value_tabletSettings.customize_verify_indication.static_unverified_info[
              "top_message"
            ] = self.value_unverifiedTopMessage_disp;
          }
          if (self.value_unverifiedBottomMessage_disp) {
            self.value_tabletSettings.customize_verify_indication.static_unverified_info[
              "bottom_message"
            ] = self.value_unverifiedBottomMessage_disp;
          }

          self.value_tabletSettings.customize_verify_indication.static_unverified_info[
            "use_qrcode"
          ] = self.value_unverifiedImageUseDynamicQrCode_disp;
          if (self.value_unverifiedImageUseDynamicQrCode_disp) {
            if (self.value_unverifiedImageQrcodeString_disp) {
              self.value_tabletSettings.customize_verify_indication.static_unverified_info[
                "qrcode_string"
              ] = self.value_unverifiedImageQrcodeString_disp;
            } else {
              self.value_tabletSettings.customize_verify_indication.static_unverified_info[
                "qrcode_string"
              ] = "";
            }
          } else {
            if (self.value_unverifiedIndicationImage_disp) {
              self.value_tabletSettings.customize_verify_indication.static_unverified_info[
                "base64_image"
              ] = self.value_unverifiedIndicationImage_disp;
            }
          }
        }

        const applyRet = await self.$globalSetTabletSetting(self.value_tabletSettings);
        if (!applyRet.error) {
          let counterRetry = 0;
          let retry = true;
          let pass = false;
          while (retry) {
            const ret = await self.$globalGetTabletSetting();
            if (!ret.error && ret.data) {
              self.value_tabletSettings = ret.data;
              self.resetSettings(ret.data);
              self.flag_dataChanged = false;
              retry = false;
              pass = true;
            } else if (counterRetry++ < 10) await self.sleep(2000);
          }
          self.$fire({
            text: pass
              ? i18n.formatter.format("Successful")
              : i18n.formatter.format("OperationFailed"),
            type: pass ? "success" : "error",
            timer: 3000,
          });
        }
        self.flag_changingSettings = false;
      },
    },
  };
</script>