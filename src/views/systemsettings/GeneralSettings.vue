  ...
  <div>
    <CCol sm="12">
      ...
      <CCard v-if="deviceProfile.support_rtsp" v-show="false">
        <CCardHeader>
          <div>
            ...
            <span class="h6 ml-2">
              <span v-if="value_rtspActivated" class="text-truncate text-black-50"> Live streaming : </span>
        ...
        <CCardHeader>
          <div>
            ...
            <span class="h6" style="display: inline-block; margin: 0; position: relative; bottom: 4px">
              <span style="background-color: white; color: gray"> Intercom client : </span>
          ...
          <CRow class="ml-2">
            <CCol sm="3">
              <CButton
                class="btn btn-primary fz-xl"
                :disabled="
                  flag_smtp_host !== '' || flag_smtp_port !== '' || flag_smtp_user !== '' || flag_smtp_pass !== ''
                "
      ...
      <CCard>
        <CCardHeader>
          <div>
            <span class="h3">{{ $t('ModelSettings') }}</span>
          </div>
        </CCardHeader>
        <CCardBody>
          <CCol sm="12">
            <div class="form-check mb-3">
              <input
                ...
                id="modelTypeLarge"
                value="big"
                v-model="value_modelType"
              />
              <label class="form-check-label h5" for="modelTypeLarge">
                {{ $t('LargeModelOption') }}
              </label>
            </div>
            <div class="form-check mb-3">
              <input
                ...
                v-model="value_modelType"
              />
              <label class="form-check-label h5" for="modelTypeSmall">
                {{ $t('SmallModelOption') }}
              </label>
            </div>
            <div class="mt-3">
              <CButton class="btn btn-primary fz-xl" @click="clickOnApplyModelSettings">
                {{ $t('Apply') }}
              </CButton>
            </div>
          </CCol>
...
export default {
  ...
  data() {
    return {
      ...
      disp_highTemperatureThreshold: `${this.$t('HighTemperatureThreshold')} ${this.$t('Celsius')}`,
  ...
  created() {
    ...
    this.$globalGetSystemSettings((err, data) => {
      if (!err) {
        const { settings } = data;
        const { db, smtp } = settings;
        if (db.verified_maintain_duration) this.value_verified_maintain_duration = db.verified_maintain_duration;
        if (db.non_verified_maintain_duration)
          this.value_non_verified_maintain_duration = db.non_verified_maintain_duration;
        ...
        if (settings.model === 'big' || settings.model === 'small') {
          this.value_modelType = settings.model;
        }
      }
    });
  },
  ...
  methods: {
    ...
    selShowTemperatureUnit(e) {
      ...
      this.disp_highTemperatureThreshold = `${this.$t('HighTemperatureThreshold')} ${
        this.value_temperatureUnitCelsius == true ? this.$t('Celsius') : this.$t('Fahrenheit')
      }`;
      this.value_highTemperatureThreshold =
        this.value_temperatureUnitCelsius == true
          ? this.value_highTemperatureThreshold_Celsius
          : this.value_highTemperatureThreshold_Fahrenheit;
    },
    ...
    isValidThreshhole(n) {
      this.flag_verifySettingsOk =
        this.isFloat(this.value_verifyThreshold) &&
    ...
    async applyCustomButton() {
      ...
      if (
        ...
        this.value_tabletSettings.custom_trigger_button_settings.enable != this.value_enableCustomButton ||
        this.value_tabletSettings.custom_trigger_button_settings.url != this.value_customFunctionUrl
      ) {
    ...
    async applyAiraManager() {
      const dataChange =
        this.$globalAiraManagerSettings.manager_host != this.value_airaManagerHostName ||
    ...
    async ApplyClockInMode() {
      ...
      if (dataChange) {
        ...
        if (!applyRet.error) {
          ...
          this.$fire({
            text: pass ? this.$t('Successful') : this.$t('OperationFailed'),
    ...
    async ApplyVerifySettings() {
      ...
      if (
        ...
        this.value_tabletSettings.anti_spoofing_score != this.value_antiSpoofingThreshold ||
        ...
        this.value_tabletSettings.face_detection_threshold != this.value_faceDetectThreshold
      ) {
        this.value_tabletSettings.score_for_valid_face = parseFloat(this.value_verifyThreshold);
        this.value_tabletSettings.anti_spoofing_score = parseFloat(this.value_antiSpoofingThreshold);
        this.value_tabletSettings.face_detection_threshold = parseFloat(this.value_faceDetectThreshold);
        this.value_tabletSettings.have_to_wear_face_mask = this.value_haveToWearFaceMask;
        this.value_tabletSettings.face_mask_enhancement = this.value_faceMaskEnhancement;
        this.value_tabletSettings.enable_id_card = this.value_enableIdCard;
        this.value_tabletSettings.enable_two_factor_authentication = this.value_enableTwoFactorAuthentication;
    ...
    async ApplyDisplaySettings() {
      ...
      if (
        ...
        this.value_tabletSettings.display_verify_result_time != Number(this.value_displayVerifyResultTime) ||
        ...
        if (!applyRet.error) {
          ...
          while (retry) {
            ...
            if (!ret.error && ret.data) {
              ...
              this.value_highTemperatureThreshold_Fahrenheit =
                (this.value_highTemperatureThreshold_Celsius * 9) / 5 + 32;
              retry = false;
              pass = true;
            } else if (counterRetry + 1 < 10) {
              counterRetry += 1;
    ...
    clickOnApplyModelSettings() {
      this.$globalSetSystemSettings({ model: this.value_modelType }, (err) => {
        if (!err) {
          this.$fire({
            text: this.$t('Successful'),
            type: 'success',
