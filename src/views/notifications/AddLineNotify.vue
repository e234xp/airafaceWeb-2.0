...
<script>
import CLineNotifyForm from './forms/LineNotifyForm.vue';

export default {
  name: 'AddLineNotify',
  components: { CLineNotifyForm },
  data() {
    return {
  ...
  created() {
    if (this.value_returnRoutePath === '') this.$router.push({ name: 'LineNotifyManagement' });
  },
  methods: {
    onFinish(data, cb) {
      this.$globalCreateLineNotify(data, (error, result) => {
