<template>
  <div>
    ...
    <stepprogress class="w-step-progress-3" icon-class="fa fa-check" :active-thickness="param_activeThickness"
      ...
      :line-thickness="param_lineThickness" :steps="[disp_inputScheduleInfo, disp_timerange, disp_complete]"
      ...
      <CCard :class="showOnStep(0)" :style="param_cardStyle">
        <CCardBody>
          <table class="table-layout">
            ...
            <tr class="table-tr">
              <td class="table-td">
                <CInput size="lg" v-model="value_scheduleName" required :invalid-feedback="disp_noEmptyNorSpaceOnly"
                  :is-valid="scheduleNameValidator" />
              </td>
              <td class="table-td">
                <CSelect size="lg" :value.sync="value_type" :options="[
        ...
        <CCardBody>
          <table class="table-layout">
            ...
            <tr class="table-tr" v-show="value_type=='recurrent'">
              <td class="table-td">
                <scheduler v-model="value_recurrentSchedule" :multiple="true" :footer="false" />
              </td>
            </tr>

            <tr class="table-tr" v-show="value_type=='non-recurrent'">
              <td class="table-td">
                <date-picker :lang="this.$globalDatePickerLanguage" v-model="value_specifiedDate" type="date" range />
                <p />
                <scheduler v-model="value_nonrecurrentSchedule" :multiple="true" :footer="false"
                  :ignore-weekend="true" />
              </td>
            </tr>
          </table>
    ...
    <CCol sm="12">
      ...
      <div class="row justify-content-center mb-4">
        ...
        <div>
          <CButton class="btn btn-primary fz-lg btn-w-normal" @click="clickOnNext" :disabled="!(flag_scheduleNamePass)">
            {{ nextButtonName() }}
          </CButton>
        </div>
      </div>
