    ...
    refreshTableItems() {
      this.$globalGetLicenseList((error, result) => {
        if (result) {
          ...
          // 每次都重新指派。只在 true 時寫入的話，後端改回 false 之後
          // 旗標會停在 true，試用按鈕就解不掉了
          this.flag_defaultLicense = Boolean(result.trial_avaiable_for_active);

          // 還能啟用試用時清單維持空的，沿用原本的行為
          if (!this.flag_defaultLicense) {
            let no = 1;
