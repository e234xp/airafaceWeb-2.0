<template>
  <div>
    <CRow>
      <CCol sm="12">
        <CPersonForm :form-data="$data" :enable-admin-field="true" :modify-mode="false" :can-modify="canModify"
          :on-finish="onFinish" />
      </CCol>
    </CRow>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      onFinish(data, cb) {
        ...
        if (data.group_list.indexOf('All Person') === -1) {
        ...
        this.$globalCreatePerson(dataForRegister, (error, result) => {
