<template>
  ...
  >
    ...
    >
      <CCarousel
        animate
        :interval="displaySettings.advertisingDuration * 1000"
        class="carousel"
        ref="carouselpanel"
      >
        <CCarouselItem
          v-for="(page, i) in displaySettings.advertising"
          :key="i"
        >
          <div
    ...
    >
      ...
      >
        <div
          v-for="(person, index) in shownPersons"
          class="welcome-card item"
          :key="index"
        >
          <img
          ...
          <div class="welcome-card-text-box">
            ...
            <div class="welcome-card-department">
              {{ showField(person, 'line3') }}
            </div>
            <div class="welcome-card-remarks">
              {{ showField(person, 'line4') }}
            </div>
          </div>
        </div>
      </transition-group>
...
export default {
  ...
  async mounted() {
    ...
    for (let idx = self.displaySettings.advertising.length - 1; idx >= 0; idx -= 1) {
      if (
        self.displaySettings.advertising[idx] === '' ||
        self.displaySettings.advertising[idx] === `data:image/png;base64,${px}`
      ) {
    ...
    initLooper() {
      ...
      self.looper = setInterval(() => {
        ...
        if (self.shownPersons.length <= self.displaySettings.numberOfDisplayPersons - 1) {
          while (
            self.shownPersons.length < self.displaySettings.numberOfDisplayPersons &&
            self.waitingPersons.length >= 1
          ) {
