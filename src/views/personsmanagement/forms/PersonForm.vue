              ...
              <CRow>
                ...
                <CCol sm="12">
                  <div class="row justify-content-center">
                    ...
                    >
                      <template #prepend>
                        <CButton :disabled="!canModify()" size="sm" color="light" @click="refreshCamera()">
                          <i class="fa fa-sync" />
                        </CButton>
                      </template>
                    </CSelect>
    ...
    async detectFaceAndGetHeadBox(img, cb) {
      ...
      return new Promise((resolve) => {
        ...
        if (detection) {
          const rangeRatio = 2;
          let headWidthToHunt =
            (detection.box.width < detection.box.height ? detection.box.width : detection.box.height) * rangeRatio;
            ...
            if (box) {
              ...
              if (this.flag_minFaceResolution) {
                try {
                  ...
                  this.value_photoToRegister = dataUrl.replace(/^data:image\/[a-z]+;base64,/, '');
          ...
          this.detectFaceAndGetHeadBox(img, (box) => {
            if (box) {
              try {
                ...
                canvasOriginal.width = box.width;
                canvasOriginal.height = box.height;
                const ctxOriginal = canvasOriginal.getContext('2d');
                ctxOriginal.drawImage(img, box.x, box.y, box.width, box.height, 0, 0, box.width, box.height);
                const dataUrlOriginal = canvasOriginal.toDataURL('image/jpeg', 1.0);
                this.value_photoToRegister = dataUrlOriginal.replace(/^data:image\/[a-z]+;base64,/, '');

                // 建立 200×200 的縮圖 (畫面顯示)
                const canvasThumb = document.createElement('canvas');
                canvasThumb.width = 200;
                canvasThumb.height = 200;
                const ctxThumb = canvasThumb.getContext('2d');
                ctxThumb.drawImage(img, box.x, box.y, box.width, box.height, 0, 0, 200, 200);
                const dataUrlThumb = canvasThumb.toDataURL('image/jpeg', 1.0);
                this.value_photoToRegisterToShow = dataUrlThumb;
              } catch (e) {
      ...
      this.selectPhotoFromFile('DISPLAY', (img) => {
        if (img) {
          ...
          this.resizeImageFromDataURL(null, img.src, 200, 200, (basr64Img) => {
            ...
            this.value_photoToDisplay = dataUrl.replace(/^data:image\/[a-z]+;base64,/, '');
    ...
    resizeImageFromDataURL(box, base64Str, maxWidth, maxHeight, callBack) {
      ...
      img.onload = () => {
        try {
          ...
          if (box) {
            ctx.drawImage(img, box.x, box.y, box.width, box.height, 0, 0, maxWidth, maxHeight);
          } else {
