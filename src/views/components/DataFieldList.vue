<template>
  <div style="overflow-y: scroll;">
    <div class="list-group-item" v-for="(item) in dataFields" :key="item.value">
      <input class="form-check-input me-1" type="checkbox" value="item" :checked="dataSelected.indexOf(item.value) >= 0"
        :disabled="!checkImage(item.value)" @change="dataFieldChanged(item.value, $event)">
      {{ $t(item.label) }}
      <CButton style="float:right; width: 40px; min-width:unset;" v-show="dataSelected.indexOf(item.value) >= 0"
    ...
    <div class="list-group-item" v-for="(item) in personFields" :key="item.value">
      <input class="form-check-input me-1" type="checkbox" value="item"
        :checked="personSelected.indexOf(item.value) >= 0" :disabled="!checkImage(item.value)"
        @change="personFieldChanged(item.value, $event)">
      {{ $t(item.label) }}
      <CButton style="float:right; width: 40px; min-width:unset;" v-show="personSelected.indexOf(item.value) >= 0"
