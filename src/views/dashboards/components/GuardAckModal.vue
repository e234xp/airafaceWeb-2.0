<template>
  <div class="modal-main">
    <div>
      ...
      <img
        :src="`data:image/png;base64,${persons[0].face_image}`"
      ...
      <div style="width: calc(100% - 240px); color: white">
        ...
        >
          ...
          <template v-else>
            <div>{{ $t('Selected') }} <span class="hint fz-xl fw-700">{{ persons.length }}</span> {{ $t('items') }}</div>
          </template>
        </div>
        <div v-if="persons.length === 1">
          ...
          >
            <img
              :src="`data:image/png;base64,${persons[0].near.register_image}`"
        ...
        <div v-if="!direct">
          ...
          <div style="display: flex; justify-content: flex-end; margin-top: 4px">
            ...
            <div
              class="remark-template"
              @click="onRemarkTemplate(opt.value)"
