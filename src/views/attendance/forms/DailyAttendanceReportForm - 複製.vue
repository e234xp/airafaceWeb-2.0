      ...
      <CCol sm="12">
        <CCol sm="12">
          ...
          <CRow class="justify-content-between buttons-group">
            ...
            <!-- 右邊按鈕群組 -->
            <div class="d-flex buttons-group-right">
              <!-- 今日 昨日 選擇 -->
              <div>
        ...
        <div>
          <div id="dailyAttendanceReportForm">
            <vxe-table :data="value_dataItemsToShow" stripe align="center" :cell-style="cellStyle"
              ...
              <vxe-table-column :show-overflow="ellipsisMode" field="person" :title="disp_person" type="html"
              ...
              <vxe-table-column :show-overflow="ellipsisMode" field="groups" :title="disp_group_list" type="html"
                width="8%" align="left"></vxe-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      async clickOnExport() {
        ...
        if (self.value_attendanceDataListToReview == null) {
          ...
          worksheet.columns = [
            ...
            { header: self.disp_clockDate, key: "clockDate", width: 10 },
            { header: self.disp_working_time, key: 'working_time', width: 10 },
            { header: self.disp_over_time, key: 'over_time', width: 10 },
            { header: self.disp_clockin, key: "clockin", width: 10 },
            { header: self.disp_clockin_temperature, key: "clockin_temperature", width: 10 },
            { header: self.disp_clockout, key: "clockout", width: 10 },
            { header: self.disp_clockout_temperature, key: "clockout_temperature", width: 10 },
            { header: self.disp_dailyAttendanceStatus, key: "attendanceStatus", width: 50 },
          ];
          ...
          for (let idx = 0; idx < self.value_allTableItems.length; idx++) {
            ...
            if (item.attendance_data_list) {
              ...
              if (item.attendance_data_list.length >= 2) {
                item["clockout"] = new Date(
                  item.attendance_data_list[item.attendance_data_list.length - 1].timestamp
                ).HHMMSS();
                item["clockout_temperature"] =
                  item.attendance_data_list[
                    item.attendance_data_list.length - 1
                  ].temperature;
              }
            }
            ...
            item["working_time"] = self.formatDurationTime(attendanceStatusData.attendance_data.working_time[i]);
            item["over_time"] = self.formatDurationTime(attendanceStatusData.attendance_data.over_time[i]);
            var status = [];

            if (item.early >= 1) status.push(i18n.formatter.format("LeaveEarly"));
            if (item.late >= 1) status.push(i18n.formatter.format("LatePerson"));
            if (item.no_record >= 1) status.push(i18n.formatter.format("NoRecord"));
          ...
          if (workbook != null) {
            workbook.xlsx.writeBuffer().then((data) => {
              var filename = `Daily_Attendance_${self.value_specifiedDate.MMdd().replace("-", "")}.xlsx`;
      ...
      async clickOnExportDetail() {
        ...
        if (workbook != null) {
          workbook.xlsx.writeBuffer().then((data) => {
            var filename = `Attendance_${self.value_specifiedDate.MMdd().replace("-", "")}_${personName}.xlsx`;
      ...
      generateFilteredData(sourceData, filter) {
        ...
        sliceList.forEach((item) => {
          ...
          if (item.attendance_data_list) {
            ...
            let clockints = item.attendance_data_list[0].timestamp;
            let clockoutts = item.attendance_data_list[item.attendance_data_list.length - 1].timestamp;
          ...
          if (attRecList) {
            ...
            if (clockInStatus == _STATUS_NONE) {
              if (passModeRecord.length > 0) {
                ...
                if ((defined_clockin_adjustment_enabled) && (firstRec.timestamp < tDayStartTs + clockInTime))
                  clockInTimeStamp = tDayStartTs + clockInTime;
                if (defined_clockin_time_late_enabled) {
            ...
            if (timeToCheckClockOut && clockOutStatus == _STATUS_NONE) {
              ...
              if (passModeRecord.length > 0) {
                ...
                if ((defined_clockout_adjustment_enabled) && (recForClockOut.timestamp > tDayStartTs + clockOutTime))
                  clockOutTimeStamp = tDayStartTs + clockOutTime;

                if (defined_clockout_time_early_leave_enabled) {
                  if (recForClockOut.timestamp >= tDayStartTs + clockOutTime - clockOutTimeBuffer) {
                    // leave on time
                    clockOutStatus = _STATUS_LEAVE_ON_TIME;
                  } else {
                    // too early to leave
            ...
            if (i == idxSpecifiedDateToCheck && clockOutStatus == _STATUS_EARLY) item.early++;
            if (i == idxSpecifiedDateToCheck && (clockInStatus == _STATUS_NO_SHOW || clockOutStatus == _STATUS_NO_SHOW)) item.no_record++;
      ...
      generateAttendanceStatusTable(attendanceData) {
        ...
        headerDataString += `<th style="text-align:center;opacity: 1.0;width:150px;font-size: 10px;border: 1px solid ${tableBordderColor}; text-overflow: ellipsis; overflow: hidden; font-family:Arial;padding: 0px 0px; color: #606266; background-color:${colorToAppend}">`;
