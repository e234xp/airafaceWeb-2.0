  ...
  <div>
      ...
      <CCol sm="12">
        <CCol sm="12">
          <CRow class="flex-row-reverse">
            <CInput v-model.lazy="value_searchingFilter" style="width: 300px" size="lg" :placeholder="$t('Search')">
          ...
          <CRow class="justify-content-between buttons-group">
            ...
            <div class="d-flex">
              ...
              <date-picker :lang="this.$globalDatePickerLanguage" style="visibility: hidden; width: 0px" type="date"
                ref="datePicker" @change="datePickerDatachange()" v-model="value_datePicked" />
        ...
        <div>
          <div id="dailyAttendanceReportForm">
            <vxe-table :data="value_dataItemsToShow" stripe align="center" :cell-style="cellStyle"
              ...
              <vxe-table-column :show-overflow="ellipsisMode" field="person" :title="$t('Person')" type="html"
                width="8%" align="left" />
              <vxe-table-column :show-overflow="ellipsisMode" field="groups" :title="$t('Group')" type="html"
                width="10%" align="left" />
              <vxe-table-column :show-overflow="ellipsisMode" field="clockDate" :title="$t('Date')" width="6%" />
              <vxe-table-column :show-overflow="ellipsisMode" field="working_time" :title="$t('WorkingTime')"
                width="10%" />
              <vxe-table-column :show-overflow="ellipsisMode" field="over_time" :title="$t('Overtime')" width="10%" />
              <vxe-table-column :show-overflow="ellipsisMode" field="clockinToShow" :title="$t('ClockInTime')"
                type="html" width="12%" align="left" />
              <vxe-table-column :show-overflow="ellipsisMode" field="clockoutToShow" :title="$t('ClockOutTime')"
                type="html" width="12%" align="left" />
              <vxe-table-column field="attendanceStatus" :title="$t('DailyAttendanceStatus')" type="html"
                min-width="300" />
        ...
        <div>
          <div>
            <vxe-table :data="value_dataItemsToShowDetailData" stripe align="center" :cell-style="cellStyle"
              ...
              <vxe-table-column :show-overflow="ellipsisMode" field="clockMode" :title="$t('ClockingMode')"
                width="12%" />
              ...
              <vxe-table-column :show-overflow="ellipsisMode" field="temperature"
                v-if="$deviceProfile.supportTemperature" :title="$t('Temperature')" width="12%" />
              <vxe-table-column field="showimage" :title="$t('CapturedPhoto')" type="html" />
              <vxe-table-column :show-overflow="ellipsisMode" field="card_number" :title="$t('CardNumber')"
                width="12%" />
            </vxe-table>
          </div>
          <vxe-pager :layouts="[
    ...
    <CModal :show.sync="flag_masterCollapse">
      ...
      <CRow v-if="false">
        ...
        <CCol col="9">
          <CSelect size="lg" :value.sync="value_snapshotFileType" disabled :options="[
      ...
      <CRow>
        ...
        <CCol col="9">
          <ul class="list-group">
            <li class="list-group-item" v-for="(item, index) in value_masterexportFields" :key="item">
    ...
    <CModal :show.sync="flag_detailCollapse">
      ...
      <CRow>
        ...
        <CCol col="9">
          <CSelect size="lg" :value.sync="value_snapshotFileType" :disabled="value_fileType !== '.xlsx'" :options="[
              { label: $t('Excluded'), value: 'Excluded' },
              { label: $t('Embedded'), value: 'Embedded' },
              { label: $t('Files'), value: 'Files' }
            ]" />
    ...
    mounted() {
      ...
      self.$globalGetAttendanceSettings((err, data) => {
        if (!err) {
          ...
          if ((self.value_workingHourSettings.video_device_group_in.length >= 1)
            || (self.value_workingHourSettings.video_device_group_out.length >= 1)) {
            self.$globalFindVideoDeviceGroups('', 0, 2000, (err, data) => {
              if (err == null) {
                const { result } = data;
