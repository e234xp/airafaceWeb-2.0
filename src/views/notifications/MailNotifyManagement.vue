<template>
  <div>
    <CRow>
      <CCol sm="12">
        <CMailNotifyManagementForm :form-data="$data" :on-add="onAdd" :on-modify="onModify" :on-delete="onDelete"
...
<script>
  import CMailNotifyManagementForm from './forms/MailNotifyManagementForm.vue';

  export default {
    name: 'MailNotifyManagement',
    components: { CMailNotifyManagementForm },
    data() {
      return {
        type: 'MailNotify',
    ...
    beforeRouteLeave(to, from, next) {
      console.log('MailNotifyManagement', to, from, next);
    ...
    methods: {
      async downloadTableItemsAsync(sliceSize, cb) {
        ...
        while (this.flag_keepingDownload && thereIsMoreData) {
          const ret = await this.$globalGetMailNotifyList(
            '', shitf, sliceSize,
          );
      ...
      async removeRecordAsync(uuid, cb) {
        const ret = await this.$globalRemoveMailNotify(uuid);
        const err = ret.error;
        if (err) {
          if (cb) cb(false);
          this.$fire({
