...
export default {
  ...
  methods: {
    ...
    updateData() {
      ...
      // 保持已選擇項目的順序（不排序，以維持拖曳調整後的順序）
      this.value_selectedItems = this.value_selectedPersonList.concat(this.value_selectedVisitorList);
    ...
    clickOnNext() {
      if (this.flag_currentSetp === 0) {
        ...
        if (this.onFinish) {
          console.log('=== 傳送到後端的順序 ===');
          console.log('person_list:', this.value_selectedPersonList.map((p, i) => `${i}: ${p.name}`));
          console.log('visitor_list:', this.value_selectedVisitorList.map((p, i) => `${i}: ${p.name}`));
