        ...
        <CRow class="mb-4">
          ...
          <CCol sm="4" class="form-inline">
            <div class="form-check" style="padding-right: 40px">
              ...
              />
              <label class="form-check-label h5" for="flexRadioDefault1">{{ disp_temperature_ALL }}</label>
            </div>
            <div class="form-check" style="padding-right: 40px">
              <input
                class="form-check-input"
                ...
                v-model="value_Temperature"
              />
              <label class="form-check-label h5" for="flexRadioDefault2">{{ disp_temperature_Normal }}</label>
            </div>
            <div class="form-check" style="padding-right: 40px">
              <input
                class="form-check-input"
                ...
                v-model="value_Temperature"
              />
              <label class="form-check-label h5" for="flexRadioDefault3">{{ disp_temperature_OverTemperature }}</label>
            </div>
          </CCol>
          <CCol sm="2" class="h5">
            {{ disp_source }}
        ...
        <CRow>
          ...
          <CCol sm="6" class="d-flex flex-wrap justify-content-end">
            <span
              v-if="exportNo >= 1"
              class="h5"
              style="float: right; height: 90%; display: flex; align-items: flex-end; margin-right: 20px"
...
export default {
  ...
  async created() {
    ...
    this.value_displayPersonSourceList = [
      ...this.cameraList.map((item) => item.name),
      ...this.tabletList.map((item) => item.name),
    ];
  ...
  methods: {
      ...
      try {
        this.value_allTableItems.forEach((pItem) => {
          ...
          try {
            item.source =
              (
                this.cameraList.find((c) => c.uuid === item.source_id) ||
                this.tabletList.find((t) => t.uuid === item.source_id)
              )?.name || '';
      ...
      try {
        self.value_allTableItems.forEach((pItem) => {
          ...
          try {
            ...
            if (showimageId && showimageId.length > 0) {
              item.showimage =
                `<img id='${showimageId}' src='data:image/jpeg;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJ` +
        ...
        if (!error) {
          ...
          self.value_allTableItems.forEach((pItem) => {
            ...
            try {
              ...
              const showimageId = item.face_image_id ? item.face_image_id.f + item.face_image_id.uuid : '';
              item.showimage =
                `<img id='${showimageId}' src='data:image/jpeg;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJA` +
      ...
      try {
        self.value_allTableItems.forEach((pItem) => {
          ...
          try {
            ...
            if (showimageId && showimageId.length > 0) {
              item.showimage =
                `<img id='${showimageId}' src='data:image/jpeg;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfF` +
    ...
    generateFilteredData(sourceData) {
      ...
      sliceList.forEach(async (slicei) => {
        try {
          const showimageId = slicei.face_image_id ? slicei.face_image_id.f + slicei.face_image_id.uuid : '';
          ...
          if (showimageId && showimageId.length > 0) {
            ...
            if (
              dataImage.error == null &&
              dataImage.data &&
              dataImage.data.face_image &&
              dataImage.data.face_image.trim() !== ''
            ) {
    ...
    async exportExcel(withPhoto) {
      ...
      for (let idx = 0; idx < self.value_allTableItems.length; idx += 1) {
        ...
        if (withPhoto) {
          await self.sleep(10);
          const faceImageRet = await self.$globalFetchVerifyPhoto(self.value_allTableItems[idx].face_image_id);
          if (!faceImageRet.error) {
            const photoId = workbook.addImage({
              base64: faceImageRet.data.face_image,
              extension: 'jpeg',
            });
            worksheet.addImage(photoId, `J${worksheet.rowCount}:J${worksheet.rowCount}`);
            ...
            const fileName = self.value_allTableItems[idx].dateTime
              ...
              .replace(' ', '_')
              .replace(':', '_');
            snapshotFolder.file(`${self.exportNo}_${fileName}.jpeg`, faceImageRet.data.face_image, { base64: true });
          }
        }
      ...
      if (workbook != null) {
        workbook.xlsx.writeBuffer().then((data) => {
          ...
          zip.generateAsync({ type: 'blob' }).then((content) => {
            FileSaver.saveAs(content, `investigation_Report_${dayjs(new Date()).format('YYYYMMDD')}.zip`);
          });
        });
      }
